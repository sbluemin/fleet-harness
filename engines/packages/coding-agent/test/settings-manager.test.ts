...
describe("SettingsManager", () => {
	...
	describe("preserves externally added settings", () => {
		it("should preserve enabledModels when changing theme", async () => {
		...
		it("should let in-memory changes override file changes for same key", async () => {
			...
			// User externally changes theme
			const currentSettings = JSON.parse(readFileSync(settingsPath, "utf-8"));
			currentSettings.theme = "light";
			writeFileSync(settingsPath, JSON.stringify(currentSettings, null, 2));

			// But then changes it via UI to a different value
			manager.setTheme("fleet-dark");
			await manager.flush();

			// In-memory change should win
			const savedSettings = JSON.parse(readFileSync(settingsPath, "utf-8"));
			expect(savedSettings.theme).toBe("fleet-dark");
		});
	});
	...
	describe("reload", () => {
		it("should default quietStartup to true when unset", () => {
			const manager = SettingsManager.create(projectDir, agentDir);
