...
import { Agent } from "@sbluemin/fleet-agent-core";
import type { Api, Model, OAuthCredentials, OAuthProvider } from "@sbluemin/fleet-ai";
import { getOAuthApiKey } from "@sbluemin/fleet-ai/oauth";
...
/**
 * Create a synthetic model fixture for tests that must not depend on built-in catalogs.
 */
...
export interface ProbeToolExtensionOptions {
	name?: string;
	label?: string;
	description?: string;
...
export function createProbeToolExtension(options: ProbeToolExtensionOptions = {}): ExtensionFactory {
	return (pi) => {
		pi.on("session_start", () => {
			const name = options.name ?? "probe_tool";
			pi.registerTool({
				name,
				label: options.label ?? "Probe Tool",
				description: options.description ?? "Tool registered from a test extension",
				promptSnippet: options.promptSnippet,
				promptGuidelines: options.promptGuidelines,
				parameters: Type.Object({}),
				execute: async (_toolCallId, _params, _signal, _onUpdate, ctx) => ({
					content: [{ type: "text", text: options.resultText ?? ctx.cwd }],
					details: {},
				}),
			});
		});
