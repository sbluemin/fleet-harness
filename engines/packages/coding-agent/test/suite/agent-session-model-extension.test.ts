...
describe("AgentSession model and extension characterization", () => {
	...
	it("applies the target model default thinking level when switching models", async () => {
		const harness = await createHarness({
			models: [
				{ id: "faux-1", name: "One", reasoning: true },
				{ id: "faux-2", name: "Two", reasoning: true },
		...
		harnesses.push(harness);
		const nextModel = harness.getModel("faux-2")!;
		nextModel.defaultThinkingLevel = "xhigh";
		nextModel.thinkingLevelMap = { xhigh: "xhigh", max: "max" };
	...
	it("cycles mapped xhigh and max thinking levels in order", async () => {
		const harness = await createHarness({ models: [{ id: "faux-1", reasoning: true }] });
		harnesses.push(harness);
		harness.session.model!.thinkingLevelMap = { xhigh: "xhigh", max: "max" };
