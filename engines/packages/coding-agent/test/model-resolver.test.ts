...
describe("parseModelPattern", () => {
	...
	describe("patterns with invalid thinking levels", () => {
		...
		test("sonnet:minimal is rejected as an invalid thinking level", () => {
			const result = parseModelPattern("sonnet:minimal", allModels);
			expect(result.model?.id).toBe("claude-sonnet-4-5");
			expect(result.thinkingLevel).toBeUndefined();
			expect(result.warning).toContain("Invalid thinking level");
			expect(result.warning).toContain("minimal");
		});
	});
...
describe("default model selection", () => {
	...
	test("findInitialModel uses the selected model default thinking level", async () => {
		const modelWithDefault: Model<"anthropic-messages"> = {
			...mockModels[0],
			defaultThinkingLevel: "high",
		};
		const registry = {
			find: () => modelWithDefault,
			getAvailable: async () => [modelWithDefault],
		} as unknown as Parameters<typeof findInitialModel>[0]["modelRegistry"];

		const result = await findInitialModel({
			scopedModels: [],
			isContinuing: false,
			defaultProvider: modelWithDefault.provider,
			defaultModelId: modelWithDefault.id,
			modelRegistry: registry,
		});

		expect(result.model).toBe(modelWithDefault);
		expect(result.thinkingLevel).toBe("high");
	});
