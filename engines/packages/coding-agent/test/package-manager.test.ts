...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("package update checks and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, CONFIG_DIR_NAME, "npm")],
				undefined,
			);
		});
		...
		it("should install missing package sources when resolving", async () => {
			...
			expect(installParsedSourceSpy).toHaveBeenCalledTimes(2);
		});

		it("should refresh temporary git sources when present", async () => {
			...
			expect(refreshTemporaryGitSourceSpy).toHaveBeenCalledWith(parsedGitSource, gitSource);
		});
		...
		it("should check package updates for configured packages", async () => {
			const installedPath = join(tempDir, CONFIG_DIR_NAME, "npm", "node_modules", "example");
			mkdirSync(installedPath, { recursive: true });
			writeFileSync(join(installedPath, "package.json"), JSON.stringify({ name: "example", version: "1.0.0" }));
			settingsManager.setProjectPackages(["npm:example"]);
			...
			expect(runCommandCaptureSpy).toHaveBeenCalled();
		});
