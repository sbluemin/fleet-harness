...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "@sbluemin/fleet-ai";
import { getApiProvider } from "@sbluemin/fleet-ai";
import { getOAuthProvider } from "@sbluemin/fleet-ai/oauth";
...
describe("ModelRegistry", () => {
	...
	describe("baseUrl override (no custom models)", () => {
		test("baseUrl-only override preserves the canonical zero-provider state", () => {
			writeRawModelsJson({
		...
		test("baseUrl-only override does not materialize unrelated providers", () => {
			writeRawModelsJson({
				anthropic: overrideConfig("https://my-proxy.example.com/v1"),
			});
		...
		test("can mix zero-provider override state and explicit custom models", () => {
			writeRawModelsJson({
			...
			const googleModels = getModelsForProvider(registry, "google");
			expect(googleModels.map((m) => m.id)).toEqual(["gemini-custom"]);
		});

		test("refresh() keeps override-only providers in the zero-provider steady state", () => {
			writeRawModelsJson({
	...
	describe("custom models merge behavior", () => {
		test("custom models without api and baseUrl no longer inherit removed built-ins", () => {
			writeRawModelsJson({
				openrouter: {
					models: [
						{
			...
			const registry = ModelRegistry.create(authStorage, modelsJsonPath);
			expect(registry.getError()).toContain('Provider openrouter: "baseUrl" is required when defining custom models.');
			expect(registry.find("openrouter", "fake-provider/fake-model")).toBeUndefined();
		});
		...
		test("custom provider names no longer merge with removed built-in catalogs", () => {
			writeModelsJson({
		...
		test("custom provider names do not materialize other removed built-in providers", () => {
			writeModelsJson({
				anthropic: providerConfig("https://my-proxy.example.com/v1", [{ id: "claude-custom" }]),
			});
			...
			expect(getModelsForProvider(registry, "google")).toEqual([]);
			expect(getModelsForProvider(registry, "openai")).toEqual([]);
		});
		...
		test("provider-level compat on override-only providers is a no-op without registered models", () => {
			writeRawModelsJson({
				openrouter: {
					compat: {
						supportsUsageInStreaming: false,
		...
		test("model schema accepts defaultThinkingLevel, thinkingLevelMap, and compat fields", () => {
			writeRawModelsJson({
				demo: {
					baseUrl: "https://example.com/v1",
					apiKey: "DEMO_KEY",
			...
			expect(model?.thinkingLevelMap).toEqual({ high: "max", max: "max" });
		...
		test("modelOverrides ignore removed built-in targets while keeping explicit custom models", () => {
			writeRawModelsJson({
				openrouter: {
			...
			expect(models.some((m) => m.id === "anthropic/claude-sonnet-4")).toBe(false);
		});
		...
		test("removing custom models from models.json returns to zero-provider state", () => {
			writeModelsJson({
	...
	describe("modelOverrides (per-model customization)", () => {
		test("model override for removed built-in target is ignored", () => {
			writeRawModelsJson({
				openrouter: {
					modelOverrides: {
						"anthropic/claude-sonnet-4": {
		...
		test("model override compat for removed built-in target is ignored", () => {
			writeRawModelsJson({
				openrouter: {
					modelOverrides: {
						"anthropic/claude-sonnet-4": {
		...
		test("model override deep merge does not create removed built-in targets", () => {
			writeRawModelsJson({
				openrouter: {
					modelOverrides: {
						"anthropic/claude-sonnet-4": {
			...
			const registry = ModelRegistry.create(authStorage, modelsJsonPath);
			expect(getModelsForProvider(registry, "openrouter")).toEqual([]);
		});

		test("multiple removed built-in model overrides are ignored together", () => {
			writeRawModelsJson({
				openrouter: {
					modelOverrides: {
						"anthropic/claude-sonnet-4": {
		...
		test("model override combined with baseUrl override stays empty without built-ins", () => {
			writeRawModelsJson({
				openrouter: {
					baseUrl: "https://my-proxy.example.com/v1",
					modelOverrides: {
		...
		test("model override cost changes are ignored for removed built-in targets", () => {
			writeRawModelsJson({
				openrouter: {
					modelOverrides: {
						"anthropic/claude-sonnet-4": {
		...
		test("model override headers do not materialize removed built-in targets", async () => {
			writeRawModelsJson({
				openrouter: {
					modelOverrides: {
						"anthropic/claude-sonnet-4": {
		...
		test("refresh() keeps removed built-in override targets absent", () => {
			writeRawModelsJson({
				openrouter: {
					modelOverrides: {
						"anthropic/claude-sonnet-4": {
		...
		test("removing model override leaves removed built-in targets absent", () => {
			writeRawModelsJson({
				openrouter: {
					modelOverrides: {
						"anthropic/claude-sonnet-4": {
	...
	describe("dynamic provider lifecycle", () => {
		test("getProviderDisplayName resolves registered, removed built-in fallback, and explicit OAuth names", () => {
			const registry = ModelRegistry.create(authStorage, modelsJsonPath);
			...
			expect(registry.find("named-provider", "demo-model")?.defaultThinkingLevel).toBe("low");
		...
		test("unregisterProvider removes custom OAuth provider without restoring removed built-ins", () => {
			const registry = ModelRegistry.create(authStorage, modelsJsonPath);
		...
		test("unregisterProvider removes custom streamSimple override without restoring removed built-ins", () => {
			const registry = ModelRegistry.create(authStorage, modelsJsonPath);
		...
		describe("dynamic provider override persistence", () => {
			test("baseUrl-only override keeps zero-provider state after refresh", () => {
				const registry = ModelRegistry.create(authStorage, modelsJsonPath);
