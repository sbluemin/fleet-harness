...
Pi ships with **no built-in LLM providers**. Register a provider via an extension or `~/.pi/agent/models.json`, then authenticate.

### Via an extension

Install or write an extension that calls `pi.registerProvider()`. See [extensions.md](extensions.md) and [custom-provider.md](custom-provider.md).

### Via models.json

Add a custom provider entry in `~/.pi/agent/models.json` (see [models.md](models.md)). For API-key providers, set credentials via environment variable or `~/.pi/agent/auth.json`:
...
You can also run `/login` and select a provider to store credentials in `~/.pi/agent/auth.json`.
