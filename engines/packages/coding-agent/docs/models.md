...
## Custom Generative AI Provider Example

Use `google-generative-ai` with a `baseUrl` to add custom models through a Generative AI compatible API:
...
Use `thinkingLevelMap` on a model to describe model-specific thinking controls. Keys are pi thinking levels: `off`, `low`, `medium`, `high`, `xhigh`, `max`.
...
Use `modelOverrides` to customize specific models without replacing the provider's full model list.
...
Behavior notes:
- `modelOverrides` are applied to the provider's existing models.
- Unknown model IDs are ignored.
- You can combine provider-level `baseUrl`/`headers` with `modelOverrides`.
- If `models` is also defined for a provider, custom models are merged after overrides. A custom model with the same `id` replaces the overridden model entry.
