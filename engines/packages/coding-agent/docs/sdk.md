...
// Custom providers must be registered via pi.registerProvider() (see extensions.md)
// or models.json (see models.md). getModel(provider, id) returns undefined
// until a provider is registered.
const myModel = getModel("my-provider", "my-model");
if (!myModel) throw new Error("Model not found");
...
const { session } = await createAgentSession({
  model: opus,
  thinkingLevel: "medium", // off, low, medium, high, xhigh, max
...
3. Environment variables registered via `setProviderEnvKey()` extension API (no built-ins by default)
...
authStorage.setRuntimeApiKey("my-provider", "...");
...
if (process.env.MY_KEY) {
  authStorage.setRuntimeApiKey("my-provider", process.env.MY_KEY);
}
