...
interface ProviderModelConfig {
  ...
  /** Default thinking level when the session has no explicit value. */
  defaultThinkingLevel?: "off" | "low" | "medium" | "high" | "xhigh" | "max";
...
If `defaultThinkingLevel` is omitted, the app uses `DEFAULT_THINKING_LEVEL` and still clamps the selected value against the model's supported thinking levels.
