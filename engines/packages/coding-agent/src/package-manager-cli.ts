...
import { getFleetUpdateStatus } from "./utils/version-check.js";
...
// 프로젝트 설정 경로 표기도 Fleet 설정 디렉터리 SSoT를 따른다.
...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|pi|${APP_NAME}] [--self] [--extensions] [--extension <source>] [--force]`;
		case "list":
...
Update Fleet and installed packages.

Options:
  --self                  Update Fleet only
  --extensions            Update installed packages only
  --extension <source>    Update one package only
  --force                 Run the Fleet self-update steps without checking behind status

Short forms:
  ${APP_NAME} update                Update Fleet and all extensions
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update ${APP_NAME}    Update Fleet only (${APP_NAME}, self, and pi are accepted)
`);
			return;
...
function printSelfUpdateUnavailable(message: string): void {
	console.error(`error: ${APP_NAME} cannot self-update this installation.`);
	console.error(message);
}

function printSelfUpdateFallback(fleetRoot: string): void {
	console.error(chalk.dim("If this keeps failing, run these commands yourself:"));
	console.error(chalk.dim(`  cd ${fleetRoot}`));
	console.error(chalk.dim("  git pull --ff-only"));
	console.error(chalk.dim("  pnpm install"));
}
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	...
	if (!fleetRoot) {
		return {
			shouldRun: false,
			unavailableReason:
				"This installation is not running from a Fleet repository checkout. Update it from the source checkout that provides this installation.",
		};
	}

	if (force) {
		return { fleetRoot, shouldRun: true };
	}

	const updateStatus = await getFleetUpdateStatus(VERSION);
	if (!updateStatus) {
		return {
			fleetRoot,
			shouldRun: false,
			unavailableReason:
				"This Fleet checkout does not have a tracked upstream branch available for git-native self-update.",
		};
	}

	if (!updateStatus.hasRemote || !updateStatus.upstream) {
		return {
			fleetRoot,
			shouldRun: false,
			unavailableReason:
	...
	if (updateStatus.behind <= 0) {
		console.log(chalk.green(`${APP_NAME} is already up to date (${updateStatus.branch})`));
		return { fleetRoot, shouldRun: false };
	}

	return { fleetRoot, shouldRun: true };
}

async function runSelfUpdate(fleetRoot: string): Promise<void> {
	const steps = [
		{ command: "git", args: ["pull", "--ff-only"], display: "git pull --ff-only" },
		{ command: "pnpm", args: ["install"], display: "pnpm install" },
	];
	console.log(chalk.dim(`Updating ${APP_NAME} from ${fleetRoot}...`));
		...
		switch (options.command) {
			...
			case "update": {
				...
				if (updateTargetIncludesSelf(target)) {
					...
					if (selfUpdatePlan.unavailableReason) {
						printSelfUpdateUnavailable(selfUpdatePlan.unavailableReason);
						process.exitCode = 1;
						return true;
					}
					...
					try {
						await runSelfUpdate(selfUpdatePlan.fleetRoot!);
					} catch (error: unknown) {
						const message = error instanceof Error ? error.message : "Unknown package command error";
						console.error(chalk.red(`Error: ${message}`));
						printSelfUpdateFallback(selfUpdatePlan.fleetRoot!);
						process.exitCode = 1;
						return true;
					}
