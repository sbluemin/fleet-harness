...
// 표시 텍스트도 실제 설정 디렉터리명과 같은 SSoT를 따른다.
const USER_RESOURCE_LABEL = `User (~/${CONFIG_DIR_NAME}/agent/)`;
const PROJECT_RESOURCE_LABEL = `Project (${CONFIG_DIR_NAME}/)`;
...
function getGroupLabel(metadata: PathMetadata): string {
	...
	if (metadata.source === "auto") {
		return metadata.scope === "user" ? USER_RESOURCE_LABEL : PROJECT_RESOURCE_LABEL;
	}
