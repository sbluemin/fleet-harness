...
function isFleetRootCandidate(dir: string): boolean {
	return FLEET_ROOT_MARKERS.every((marker) => existsSync(join(dir, marker)));
}

function findFleetRootFrom(startDir: string): string | undefined {
	let dir = resolve(startDir);
	while (true) {
		if (isFleetRootCandidate(dir)) {
			return dir;
		}
...
function getGitTopLevel(cwd: string): string | undefined {
	...
	const resolvedGitRoot = resolve(result.stdout.trim());
	return isFleetRootCandidate(resolvedGitRoot) ? resolvedGitRoot : undefined;
}
...
/**
 * Resolve the Fleet repository root for repo-local installs.
 * Uses `git rev-parse --show-toplevel` first, then falls back to marker-based ancestor scans.
 */
export function getFleetRoot(): string | undefined {
	const startDirs = [getPackageDir(), process.cwd(), __dirname];

	for (const startDir of startDirs) {
		const gitRoot = getGitTopLevel(startDir);
		if (gitRoot) {
			return gitRoot;
		}
		const fallbackRoot = findFleetRootFrom(startDir);
		if (fallbackRoot) {
			return fallbackRoot;
		}
	}
...
export function getChangelogPath(): string | undefined {
	...
	return fleetRoot ? resolve(join(fleetRoot, "CHANGELOG.md")) : undefined;
}
...
export const PACKAGE_NAME: string = pkg.name || "@sbluemin/fleet-coding-agent";
export const APP_NAME: string = piConfigName || "fleet";
export const APP_TITLE: string = piConfigName ? APP_NAME : "π";
export const CONFIG_DIR_NAME: string = pkg.piConfig?.configDir || ".fleet";
...
// =============================================================================
// User Config Paths (~/.fleet/agent/*)
// =============================================================================

/** Get the agent config directory (e.g., ~/.fleet/agent/) */
