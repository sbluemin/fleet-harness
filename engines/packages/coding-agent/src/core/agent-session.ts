...
import type { AssistantMessage, ImageContent, Message, Model, TextContent } from "@sbluemin/fleet-ai";
import {
	clampThinkingLevel,
...
import { VALID_THINKING_LEVELS } from "../cli/args.js";
...
import type { BashOperations } from "./extensions/types.js";
...
export interface AgentSessionConfig {
	...
	/** Initial active tool names. Default: none. */
...
export class AgentSession {
	...
	private async _cycleScopedModel(direction: "forward" | "backward"): Promise<ModelCycleResult | undefined> {
		...
		// - 명시된 scoped model thinking level은 모델 기본값보다 우선한다.
		// - 명시값이 없으면 대상 모델의 기본 thinking level을 적용한다.
		// setThinkingLevel이 모델 capability에 맞춰 보정한다.
		this.setThinkingLevel(thinkingLevel);
	 ...
	 * Saves to session only if the level actually changes.
	 */
	...
	private _getThinkingLevelForModelSwitch(targetModel: Model<any>, explicitLevel?: ThinkingLevel): ThinkingLevel {
		...
		return (targetModel.defaultThinkingLevel ?? DEFAULT_THINKING_LEVEL) as ThinkingLevel;
	}
	...
	async executeBash(
		...
		if (!options?.operations) {
			throw new Error("Bash execution requires extension-provided operations.");
		}
		...
		try {
			const result = await options.operations.execute(resolvedCommand, this.sessionManager.getCwd(), {
				onChunk,
