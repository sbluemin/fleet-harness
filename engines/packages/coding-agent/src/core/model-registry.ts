...
import { registerOAuthProvider, resetOAuthProviders } from "@sbluemin/fleet-ai/oauth";
...
const ProviderCompatSchema = Type.Unknown();
...
function mergeCompat(
	...
	if (isRecord(baseCompat) && isRecord(overrideCompat)) {
		return { ...baseCompat, ...overrideCompat } as Model<Api>["compat"];
	}
...
export class ModelRegistry {
	...
	static create(authStorage: AuthStorage, modelsJsonPath: string = join(getAgentDir(), "models.json")): ModelRegistry {
		return ModelRegistry.instantiate(authStorage, modelsJsonPath);
	}

	static inMemory(authStorage: AuthStorage): ModelRegistry {
		return ModelRegistry.instantiate(authStorage, undefined);
	}
	...
	private static instantiate(authStorage: AuthStorage, modelsJsonPath: string | undefined): ModelRegistry {
		return Reflect.construct(ModelRegistry, [authStorage, modelsJsonPath]) as ModelRegistry;
	}
	...
	private parseModels(config: ModelsConfig): Model<Api>[] {
		...
		const getBuiltInDefaults = (providerName: string): { api: string; baseUrl: string } | undefined => {
			...
			const builtIn = getModels(providerName) as Model<Api>[];
