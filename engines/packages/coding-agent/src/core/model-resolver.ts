...
import { type Api, type Model, modelsAreEqual } from "@sbluemin/fleet-ai";
	...
	if (cliProvider && cliModel) {
		...
		if (resolved.model) {
			return {
				...
				thinkingLevel:
					resolved.thinkingLevel ?? ((resolved.model.defaultThinkingLevel ?? DEFAULT_THINKING_LEVEL) as ThinkingLevel),
	...
	if (scopedModels.length > 0 && !isContinuing) {
		return {
			...
			thinkingLevel:
				...
				((scopedModels[0].model.defaultThinkingLevel ?? DEFAULT_THINKING_LEVEL) as ThinkingLevel),
			fallbackMessage: undefined,
		};
	}
	...
	if (availableModels.length > 0) {
		...
		for (const provider of Object.keys(defaultModelPerProvider)) {
			...
			if (match) {
				return {
					model: match,
					thinkingLevel: (match.defaultThinkingLevel ?? DEFAULT_THINKING_LEVEL) as ThinkingLevel,
					fallbackMessage: undefined,
				};
			}
		}
		...
		return {
			...
			thinkingLevel: (availableModels[0].defaultThinkingLevel ?? DEFAULT_THINKING_LEVEL) as ThinkingLevel,
			fallbackMessage: undefined,
		};
	}
