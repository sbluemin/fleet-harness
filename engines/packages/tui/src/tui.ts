...
// Fleet 브랜딩 디렉터리명을 패키지 메타에서 읽고 실패 시 .fleet으로 고정한다.
const CONFIG_DIR_NAME =
	(JSON.parse(fs.readFileSync(new URL("../package.json", import.meta.url), "utf-8")) as { piConfig?: { configDir?: string } })
		.piConfig?.configDir || ".fleet";
const AGENT_DIR = path.join(os.homedir(), CONFIG_DIR_NAME, "agent");
...
export class TUI extends Container {
	...
	private doRender(): void {
		...
		const logRedraw = (reason: string): void => {
			...
			const logPath = path.join(AGENT_DIR, "pi-debug.log");
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(AGENT_DIR, "pi-crash.log");
				const crashData = [
