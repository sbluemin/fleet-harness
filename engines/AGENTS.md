...
- 이 디렉토리의 모든 패키지(예: `unified-agent`)는 Fleet 도메인 식별자·로직·의미를 코드·타입·상수·문서에 포함하지 않는다.
...
| 로컬 패키지명 | 엔진 경로 | 버전 |
|---------------|---------------|------|
| `@sbluemin/fleet-unified-agent` | `packages/unified-agent/` | `0.74.0` |

## Scope Boundaries

- `fleet-core`는 이 디렉토리를 직접 import하지 않습니다 — 오직 `workspace:*` 링크를 통해 종속성으로 참조합니다.
- `fleet-agent`는 `fleet-core`의 public API를 통해서만 이 런타임에 접근합니다.
- 이 디렉토리 내에서 외부 에이전트 전용 패키지로의 직접 import는 허용되지 않습니다.
