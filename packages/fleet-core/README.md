...
Fleet-owned runtime data defaults to `.fleet`, including the active auth store at `~/.fleet/auth.json` and the fleet data directory at `~/.fleet/`. The former `~/.fleet/agent/auth.json` path is treated only as a merge-only migration input.
