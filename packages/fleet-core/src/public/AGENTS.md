...
2. **서비스 컨테이너 금지** — `admiral`, `admiralty`, infra 서비스를 묶어 반환하는 공개 runtime context를 만들지 않는다.
3. **facade 직접 소비** — 소비자는 `@sbluemin/fleet-core` root barrel의 `admiral`, `admiralty` facade를 직접 import하고, infra는 `@sbluemin/fleet-infra`에서 import한다.
...
- `admiral`, `admiralty`, infra 서비스를 재포장하는 runtime/service container 타입
