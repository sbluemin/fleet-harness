...
- `carrier/`, `carrier-jobs/`, `taskforce/` — carrier framework, fleet tool specs, roster rendering, and execution doctrine. Default carrier persona data is owned by `packages/fleet-carriers`.
...
- **Prefix-based policy enforcement**: Sub-operation byte caps and formatting are consistently applied via `jobId` prefix detection (`taskforce:`), ensuring architectural intent is preserved even when ephemeral job metadata is evicted from memory.
