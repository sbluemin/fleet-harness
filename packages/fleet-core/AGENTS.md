...
- Carrier registration does not mutate executor MCP authorization state. Carrier-centric executor tool access is declared as opaque strings in `CarrierMetadata.allowedExecutorTools` and resolved lazily by `getExecutorMcpTools(carrierId)` alongside the global `carrier_jobs` scope and tool-centric whitelist entries.
