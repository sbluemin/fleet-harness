...
- Pi-specific lifecycle coordination (`src/boot.ts`, `src/fleet.ts`). `bootstrapFleetState(pi)` is the single entry point for both cold boot and warm `session_start`; it composes `restoreFleetPreRegistrationState()`, carrier registration, and deferred reconciliation. `scheduleFleetReconciliation()` (formerly `scheduleFleetBootReconciliation`) defers model reconciliation and Task Force sync into the next tick with a re-entrancy guard. `restoreFleetPreRegistrationState()` always applies stored sortie state, including empty lists, to reset stale in-memory Sets.
...
- `scheduleFleetReconciliation()` (formerly `scheduleFleetBootReconciliation`) defers model reconciliation and Task Force sync into the next tick with a re-entrancy guard.
