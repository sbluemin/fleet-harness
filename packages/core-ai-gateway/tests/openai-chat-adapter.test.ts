...
describe("opencode go wire routing", () => {
  ...
  it("builds the matching adapter and endpoint per translated wire", () => {
    expect(createOpencodeGoAdapter("responses")).toBeInstanceOf(OpenAIResponsesAdapter);
    expect(createOpencodeGoAdapter("chat-completions")).toBeInstanceOf(OpenAIChatCompletionsAdapter);
  ...
  it("sends chat-wire requests to the Go chat completions endpoint", async () => {
    const fetchMock = vi.fn<typeof fetch>(async () => sse("data: [DONE]\n\n"));
    ...
    expect(String(fetchMock.mock.calls[0]?.[0])).toBe(OPENCODE_GO_CHAT_COMPLETIONS_URL);
  });
});
