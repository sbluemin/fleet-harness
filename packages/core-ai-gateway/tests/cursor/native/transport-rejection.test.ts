...
import { AgentServerMessageSchema } from "../../../src/cursor/native/generated/cursor-agent-protobuf.js";
...
describe("Cursor upstream rejection", () => {
  ...
  it("caps a rejected Run's error body instead of buffering whatever Cursor sends", async () => {
    const response = await runCursorTransport({
      head: { ":status": 502, "content-type": "text/html" },
      chunks: [Buffer.alloc(40 * 1024, 0x61), Buffer.alloc(40 * 1024, 0x62)],
    });

    expect(response.ok).toBe(false);
    if (response.ok) throw new Error("unreachable");
    expect(response.body.byteLength).toBe(64 * 1024);
  });

  it("fails a 2xx whose body is not Connect framing instead of completing an empty turn", async () => {
    const events: CursorDiagnosticEvent[] = [];
    const response = await runCursorTransport({
      head: { ":status": 200, "content-type": "application/json" },
      chunks: [Buffer.from(JSON.stringify({ error: { code: "unauthenticated" } }), "utf8")],
      diagnostics: (event) => events.push(event),
    });

    expect(response.ok).toBe(true);
    if (!response.ok) throw new Error("unreachable");
    await expect(drain(response.events)).rejects.toThrow(/produced no frames/);
    expect(turnFinish(events)).toEqual(expect.objectContaining({
      outcome: "stream_end_without_frames",
      error: "empty_stream",
      frameCount: 0,
    }));
  });
  ...
  it("surfaces a compressed Connect frame instead of decoding it", async () => {
    ...
    expect(response.ok).toBe(true);
    if (!response.ok) throw new Error("unreachable");
    await expect(drain(response.events)).rejects.toThrow(/Compressed Cursor Connect frames/);
  });

  it("surfaces a Connect end-stream error frame as the turn's failure", async () => {
    const response = await runCursorTransport({
      chunks: [endStreamFrame({ error: { code: "resource_exhausted", message: "at capacity" } })],
    });

    expect(response.ok).toBe(true);
    if (!response.ok) throw new Error("unreachable");
    await expect(drain(response.events)).rejects.toThrow(/resource_exhausted: at capacity/);
  });

  it("surfaces a malformed Connect end-stream payload instead of completing", async () => {
    const response = await runCursorTransport({
      chunks: [encodeConnectFrame(Buffer.from("}not json{"), CONNECT_FLAG_END_STREAM)],
    });

    expect(response.ok).toBe(true);
    if (!response.ok) throw new Error("unreachable");
    await expect(drain(response.events)).rejects.toThrow(/invalid JSON/);
  });

  it("still completes a Run whose only frame is a clean end-stream envelope", async () => {
    // The zero-frame guard must not reject a protocol-conformant close. Cursor said the Run
    // finished; an empty turn from a well-formed exchange is not this gate's concern.
    const response = await runCursorTransport({ chunks: [endStreamFrame({})] });

    expect(response.ok).toBe(true);
    if (!response.ok) throw new Error("unreachable");
    await expect(drain(response.events)).resolves.toEqual([]);
  });

  it("decodes a Connect frame delivered across chunk boundaries", async () => {
    const frame = serverFrame({ interactionUpdate: { textDelta: { text: "split" } } });
    const response = await runCursorTransport({
      chunks: [
        frame.subarray(0, 3),
        Buffer.concat([
          frame.subarray(3),
          serverFrame({ interactionUpdate: { turnEnded: {} } }),
        ]),
      ],
    });
    ...
    expect(response.ok).toBe(true);
    if (!response.ok) throw new Error("unreachable");
    const events = await drain(response.events);
