...
- The OpenCode Chat Completions implementation takes the opposite route: it prunes undeclared keys from *completed* tool-call arguments and rewrites no schema outbound. That wire accepts `strict: true` and ignores it — measured against `deepseek-v4-flash` on 2026-08-07, where 5 of 5 runs returned an undeclared key under a closed, fully-required schema — so an outbound rewrite buys nothing there while still risking the 400 above. Prune only closed objects (`additionalProperties: false`), leave every subtree the walker cannot resolve (`$ref`, a branching keyword, absent `properties`) exactly as it arrived, and return the original bytes when nothing was dropped. The asymmetry decides the doubtful cases: a key wrongly dropped is silent data loss, a key wrongly kept is the behaviour that already shipped. This is inbound-only and schema-declared-only — it never suppresses a *declared* argument, which the strict-mode rule above still forbids.
