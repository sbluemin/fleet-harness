...
- **Forbidden Workspace Dependencies**: `@dotobokuri/fleet-carriers`, `@dotobokuri/fleet-agent`, `@dotobokuri/fleet-wiki`, `@dotobokuri/fleet-wiki-web`, and `@dotobokuri/fleet-unified-agent`.
...
- Fleet PTY internals such as region stacks and overlay managers are internal implementation details. Public consumers use `@dotobokuri/fleet-tui/pty`.
...
- **Mouse contract**: optional Fleet PTY component `handleMouse(event)` receives generic SGR-derived mouse events with 1-based component-local row/column coordinates. Default Fleet PTY regions consume mouse events as no-op; overlays may handle and still consume by default.
...
- **Unchanged invariants** (DIR-1 does not touch): `@dotobokuri/fleet-tui/input` tokenization/routing, key-encoding identity passthrough, and `@xterm/headless` `disableStdin: true` on the Dedicated bridge.
