...
describe("wiki patch queue", () => {
  ...
  it("rejects template section regressions at approval time", async () => {
    const root = await makeTempRoot();
    const paths = resolveMemoryPaths(root);
    const patch = await parsePatch(`---\nop: "create_wiki"\ntarget: "wiki/guide-approval.md"\nsummary: "Guide approval"\nproposer: "test"\ncreated: "2026-04-26T00:00:00.000Z"\n---\n{"id":"guide-approval","title":"Guide approval","tags":[],"created":"2026-04-26T00:00:00.000Z","updated":"2026-04-26T00:00:00.000Z","version":1,"body":"## Overview\\n\\nlong enough approval body without the required related section"}`);
    const patchId = await enqueuePatch(patch, paths);

    await expect(approvePatch(patchId, paths)).rejects.toThrow(/template approval failed.*missing sections: Related/);
  });
  ...
  it("approves a full patch set and records aggregate approval", async () => {
    ...
    const sourcePatch = await parsePatch(`---\nop: "create_wiki"\ntarget: "wiki/sources/source-alpha.md"\nsummary: "Source Alpha"\nproposer: "test"\ncreated: "${createdAt}"\n---\n{"id":"source-alpha","title":"Source Alpha","tags":["source"],"created":"${createdAt}","updated":"${createdAt}","version":1,"type":"source","status":"current","templateId":"guide","body":"## Overview\\n\\nsource body\\n\\n## Related\\n\\n- None"}`);
