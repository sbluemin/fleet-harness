...
- The approach is settled and only the work remains. Use `implementation-run`.
- Judging code that exists against a standard. Use `quality-review`.
- Gathering facts with no decision attached. Use `codebase-research`.
...
| Judge | verify | one per approach, mixed lineage | A score per axis with the reason, not a verdict alone |
...
- **Mixed lineage in Judge.** Judges that share a model lineage with each other, or with the session model, share blind spots. This is the one stage where lineage diversity earns its cost.
