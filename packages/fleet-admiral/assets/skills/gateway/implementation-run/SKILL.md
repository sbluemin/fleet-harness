...
The only stage shape here that **writes**. Its risk is not failure — a failed edit is visible — but convergence: many subagents each producing something reasonable that together do not match the codebase.

Model and effort assignment belongs to `model-loadout`; this skill owns the shape of the run.

## When Not To Use

- The approach is undecided. Decide first with `architecture-review`; a subagent handed an open decision will close it for you, differently in each branch.
- A handful of files you can edit directly. The per-agent overhead exceeds the work.
...
| **Decide** | — | **host only** | The literal values every site will use. Never delegated. |
...
1. Must the subagent choose a concrete value?
...
Never delegate a choice behind phrases like "match the existing style", "pick a consistent name", "follow the convention", or "적절히". A subagent on another model has no feel for this repository and will produce something defensible but foreign.
...
- **Inspect artifacts, never narratives.** Read the actual diff for each site. A subagent's summary of what it did is evidence of what it believed, not of what it wrote.
