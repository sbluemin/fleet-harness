...
- Fleet Plans: the host owns planning and all Plan mutation, including completion marking after artifact inspection; load ${"`"}plan-operations${"`"} before the first host ${"`"}plan_write${"`"} call in a session and skip reloading when already in context. Nimitz may optionally audit an existing host-authored PlanRef when plan_ref is supplied. Host plan_read once per operation for full context; Genesis plan_read once per Plan-driven dispatch with one host-authored same-Lane TaskRef group. Re-read only on Plan-state conflict or explicit redirection; plan_verify proves Plan state, not code correctness.
