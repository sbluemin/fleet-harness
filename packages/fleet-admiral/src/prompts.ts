...
- Fleet Plans: the host owns planning and Plan mutation except Ohio's completion marking; load ${"`"}plan-operations${"`"} before the first host ${"`"}plan_write${"`"} call in a session and skip reloading when already in context. Kirov may only audit an existing host-authored PlanRef. Host plan_read once per operation for full context; Ohio plan_read once per dispatch with one host-authored same-Lane TaskRef group. Re-read only on Plan-state conflict or explicit redirection; plan_verify proves Plan state, not code correctness.
