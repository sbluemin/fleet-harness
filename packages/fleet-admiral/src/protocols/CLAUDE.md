...
- Gateway assignment fills judgment seats by catalog quality evidence — `benchmark` figures where measured, the `capabilityClass` prior where not — and spreads mechanical fans by provider allowance, weighted by the user's opt-in `providerPriority`; spending the session's own allowance is the exception and must record which labelled exception applied. Quality signals live in the core-ai-gateway catalog, never in a fleet-admiral side table.
