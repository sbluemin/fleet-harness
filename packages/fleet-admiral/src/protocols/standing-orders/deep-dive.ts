...
const DEEP_DIVE_SHARED_PREFIX = String.raw`## Deep Dive Standing Order
...
const DEEP_DIVE_CLASSIC_AUDIT = String.raw`   - Select the Carrier whose role fits the audit task and dispatch it via ${"``"}carrier_dispatch${"``"}.
...
const DEEP_DIVE_GATEWAY_AUDIT = String.raw`   - Select the Workflow agent stage whose role fits the audit task and run that stage via the Workflow tool.
   - In either case, provide explicit instructions: *"Review the following analysis for speculative, assumed, or unverified claims. Flag each with evidence of why it is speculative and what verification is needed."*
3. **Follow-up verification** — For each identified speculative element:
   - Run an appropriate Workflow agent stage via the Workflow tool to seek independent confirmation or refutation.
`;

const DEEP_DIVE_SHARED_SUFFIX = String.raw`4. **Repeat** until all speculative elements are either **confirmed with evidence** or explicitly flagged as **unresolvable unknowns**.
...
export const DEEP_DIVE: StandingOrder = {
  id: "deep-dive",
  name: "Deep Dive",
  prompt: `${DEEP_DIVE_SHARED_PREFIX}${DEEP_DIVE_CLASSIC_AUDIT}${DEEP_DIVE_SHARED_SUFFIX}`,
};

/** Gateway doctrine: audit and follow-up verification via Workflow agent stages. */
export const DEEP_DIVE_FOR_GATEWAY: StandingOrder = {
  id: "deep-dive",
  name: "Deep Dive",
  prompt: `${DEEP_DIVE_SHARED_PREFIX}${DEEP_DIVE_GATEWAY_AUDIT}${DEEP_DIVE_SHARED_SUFFIX}`,
};
