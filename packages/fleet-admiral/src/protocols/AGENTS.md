...
- Gateway identities are already registered in the session, so the doctrine describes a run as a call that returns. Queue, job, poll, detach, and `<system-reminder>` vocabulary carried the old MCP carrier model and must not return to the gateway prompt or its skill assets.
