...
const GATEWAY_MODELS_DOCTRINE = {
  ...
  usageGuidelines: [
    ...
    `Quality reads benchmark first — third-party figures measured about the vendor model, with scores inside routingTieBandPoints forming one band and a caveat changing what its figures are evidence of — and capabilityClass where unmeasured: judgment seats keep to the top reachable band, and neither quality nor allowance ever falls back to this session's own model. Scores compare only within one source; a cross-source candidate is read by its standing among its own source's comparators, and class breaks what standing cannot.`,
