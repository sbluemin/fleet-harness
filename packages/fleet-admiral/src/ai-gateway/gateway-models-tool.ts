...
const GATEWAY_MODELS_DOCTRINE = {
  ...
  usageGuidelines: [
    ...
    `Read pressure as the verdict on one window, combining headroom with burn pace against that window's own clock, and it never says which model to choose. It outranks arithmetic of your own: "ok" is usable at any percentage, "elevated" is a reason to rebalance rather than a prohibition, and only "critical" excludes a provider unless every alternative is worse — re-deriving risk from usedPercent or paceRatio to overrule the verdict is how a healthy provider gets abandoned. Compare usedPercent only within one cadence — a shared window id does not mean a shared length — and only to break a tie the verdicts left even. roleFit.tokenEfficiency is a different axis entirely: tokens and tool calls are a model's appetite, not a subscription's drain, and quota is never inferred from them. Where a provider splits into pools, quotaScope picks the window that applies and the isAggregate one stays out of headroom math. recoveryHalfLifeMs prices the drain: weeks of lockout for a monthly window, hours for a session one.`,
