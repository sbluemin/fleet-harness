...
Infrastructure substitutions are intentionally narrow: Pi `ctx.ui.custom` becomes `fleetPty.custom`, fleet-tui component/key/width helpers become local `tui/pty/fleet` and `tui/primitives/cell-width`, and `pi.registerShortcut` becomes local `input/keybindings.register`.
