...
export function createProgrammaticInput(ptyHost: PtyHost, profile: DedicatedCliProfile): ProgrammaticInput {
  return {
...
): Required<NonNullable<DedicatedCliProfile["messagePolicy"]>> {
  return {
...
function applyMessagePolicy(
  text: string,
  policy: Required<NonNullable<DedicatedCliProfile["messagePolicy"]>>,
): string {
