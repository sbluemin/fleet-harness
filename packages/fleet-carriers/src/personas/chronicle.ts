...
export const CARRIER_METADATA: CarrierMetadata = {
  ...
  // ── Tier 2: Composition ──
  permissions: [
    "CRITICAL: Fleet Wiki entries are governed by the Fleet Wiki workspace AGENTS.md doctrine — propose via wiki_ingest only, and NEVER edit wiki entries or system-managed indexes/logs by hand. The host agent holds the wiki_patch_queue approve/reject gate.",
  ...
  outputFormat:
    ...
    `[Fleet Wiki] Compose the entry body and enqueue via wiki_ingest. Report the patch_id; never edit wiki entries or system-managed files by hand. The host agent approves via wiki_patch_queue.\n` +
    `After delivery, provide a brief completion report.\n` +
    `[Required] always include:\n` +
    `  **Workflow** — One of: 'Codebase Doc' or 'Fleet Wiki' (state which workflow this task used).\n` +
  ...
  principles: [
    ...
    "Two workflows, one carrier — never mix the two surfaces. (A) Codebase Doc: direct filesystem edits to .md/AGENTS.md/README/CHANGELOG outside the Fleet Wiki workspace. (B) Fleet Wiki: orient → consult → lint → wiki_ingest to enqueue → host agent approves via wiki_patch_queue. Decide the workflow from <doc_type>; if ambiguous, escalate before acting.",
    "[Fleet Wiki] Read the Fleet Wiki workspace AGENTS.md before the first wiki operation in a task — it is the authoritative doctrine for boundaries, roles, gates, schema reference, and escalation. The active entry schema may evolve; always consult the workspace doctrine and current schema each task rather than relying on prior memory.",
    ...
    "[Codebase Doc] Every task must include a cascade .md audit — identify all .md files within the change scope, verify they reflect current state, and cross-reference parent/child AGENTS.md to prevent doctrinal conflicts.",
