...
`packages/` is the Fleet first-party workspace monorepo root, containing `core-agent`, `core-unified-agent`, `fleet-infra`, `fleet-admiral`, `fleet-carriers`, and `fleet-wiki`. The `fleet-cli` and `fleet-wiki-ui` workspaces live under `runtime/`.
