...
| Executor MCP tool exposure | `admiral/agent/tools.ts:getExecutorMcpTools()` | Whitelist-only connect-time MCP exposure for `executeWithPool` / `executeOneShot`; `carrier_jobs` is global `*`. Tool access may be scoped tool-centrically via `registerExecutorTool(..., { allowedCarriers })` or carrier-centrically via `CarrierMetadata.allowedExecutorTools`; `getExecutorMcpTools()` lazy-unions both with global tools and returns only registered specs. `EXECUTOR_MCP_TOOL_IDS` is audit/compat enumeration, not the sole authorization source. |
