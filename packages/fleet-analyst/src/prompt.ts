...
The host wraps your HTML in a document that already sets the page ground, text color, color-scheme, outer page padding, and a base stylesheet covering type, lists, tables, code, details, and citations. Design on top of that host; do not rebuild it and do not add your own page-level margin.

Theme. Every color you set comes from an injected token with a literal fallback, written as var(--fleet-ink, #e8e8e8). The tokens are --fleet-canvas (page ground), --fleet-surface (inset panel, code, and chip ground), --fleet-ink (primary text), --fleet-muted (secondary text), --fleet-hairline (borders), --fleet-accent (links and emphasis), --fleet-positive, --fleet-warn, --fleet-critical (state), --fleet-focus (keyboard focus and location), and --fleet-sans, --fleet-mono (type). Never invent a page palette, never set a bare literal color, and never branch on prefers-color-scheme: the Console theme is chosen inside the Console and is independent of the OS theme, so a media query paints the wrong theme's colors. State color is not the accent; --fleet-accent never means "good".
