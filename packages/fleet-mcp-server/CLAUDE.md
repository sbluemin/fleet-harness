...
- Keep this package leaf-only: no `@sbluemin/fleet-*`, Fleet engine, Anthropic, or MCP SDK dependencies.
- Own only generic MCP server, routing, snapshot, registry, type, formatter, and invocation primitives.
- Fleet carrier metadata, default tool builders, single-fleet prompt composition, and runtime composition stay in `fleet-agent/src/admiral`; Grand Fleet composition stays in `fleet-agent/src/grand-fleet`.
