...
describe('cli-repl /effort', () => {
  it('cursor는 unsupported provider이므로 /effort high를 안내 후 무시한다', async () => {
    ...
    expect(writes.join('')).toContain('cursor/auto 모델은 reasoning effort를 지원하지 않아 /effort high 를 무시합니다');
  });
