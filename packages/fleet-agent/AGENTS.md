...
- **Must Own**: local host assembly, host `controls/**`, host `sections/**`, carrier-status domain wiring, dedicated CLI profile resolution, and CLI process lifecycle.
...
`@sbluemin/fleet-harness` and `engines/*` are permanently forbidden dependencies. The allowed `@sbluemin/fleet-tui` package is the first-party package under `packages/fleet-tui`, not the engines-era workspace dependency.
...
- **Fleet PTY**: Lower pane, backed by generic `@sbluemin/fleet-tui/pty`.
  Host-composed default content lives in `src/sections/default-sections.ts`.
