...
describe("daemon workspaces", () => {
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "fleet-wiki-web-daemon-"));
  ...
  it("registers a second workspace and updates the memory-backed MRU list", async () => {
    const second = await mkdtemp(path.join(os.tmpdir(), "fleet-wiki-web-daemon-second-"));
  ...
  it("rejects admin registration for directories without .fleet/knowledge", async () => {
    const missing = await mkdtemp(path.join(os.tmpdir(), "fleet-wiki-web-daemon-missing-"));
    try {
      const response = await fetch(`${baseUrl}/api/admin/workspaces`, {
        method: "POST",
        headers: {
