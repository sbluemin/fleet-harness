...
- A Fleet Routing panel shows each delegated run from the moment it starts: what was requested, which model carried it, and why that model was chosen. Open it any time with `/fleet-routing`.
  ko: Fleet Routing 판이 위임 실행을 시작 시점부터 보여줍니다 — 무엇을 요청했고, 어느 모델이 실행했으며, 그 모델이 선택된 이유가 표시됩니다. `/fleet-routing`으로 언제든 열 수 있습니다.
