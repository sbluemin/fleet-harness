...
- Write actions guard themselves before running: the panel reports a locked index, an in-progress merge, rebase, or cherry-pick, and Operations stationed in the same checkout, and locks its write verbs while those hold.
  ko: 쓰기 동작이 실행 전에 스스로 울타리를 칩니다. 잠긴 인덱스, 진행 중인 merge/rebase/cherry-pick, 같은 체크아웃에 주둔한 Operation을 패널이 알리고 그동안 쓰기 동사를 잠급니다.
