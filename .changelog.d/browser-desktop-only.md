...
### fleet-console
#### Changed
- The Operation Browser is now a Fleet Desktop feature: tabs render as real browser views inside the Desktop window, whether that window shows your own Console or a remote one you joined. Console no longer looks for a Chrome installation, so the browser engine path setting and the Chrome cookie import are gone.
  ko: Operation 브라우저가 Fleet Desktop 전용 기능이 됩니다. 탭은 Desktop 창 안의 실제 브라우저 뷰로 그려지며, 그 창이 내 Console을 보든 원격으로 접속한 Console을 보든 같습니다. Console이 더 이상 Chrome 설치를 찾지 않으므로 브라우저 엔진 경로 설정과 Chrome 쿠키 가져오기는 사라집니다.
