...
### fleet-cli
#### Added
- Record third-party benchmark evidence (CursorBench and SWE-rebench) in the gateway model catalog and report it through the `gateway_models` roster; sessions rank judgment work by measured scores first, fall back to the provider class only where no figures exist, and never compare scores across sources.
  ko: 게이트웨이 모델 카탈로그에 제3자 벤치마크 실측치(CursorBench·SWE-rebench)를 기록하고 `gateway_models` 로스터로 보고합니다. 세션은 판단 작업을 측정 점수 우선으로 배정하고 수치가 없는 모델만 공급자 등급으로 폴백하며, 점수는 소스 간에 비교하지 않습니다.
...
### fleet-console
#### Added
- Record third-party benchmark evidence (CursorBench and SWE-rebench) in the gateway model catalog; Console-launched sessions rank judgment work by measured scores first, and each gateway agent identity description carries its benchmark figures.
  ko: 게이트웨이 모델 카탈로그에 제3자 벤치마크 실측치(CursorBench·SWE-rebench)를 기록합니다. Console에서 띄운 세션은 판단 작업을 측정 점수 우선으로 배정하고, 게이트웨이 에이전트 정체성 description에 벤치마크 수치가 담깁니다.
