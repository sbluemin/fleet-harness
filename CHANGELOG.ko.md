...
#### Added
- 실험 기능 「Operation 위치 표시」: 사이드바 Operation 칩에 git 브랜치를, 에이전트가 Theater 루트 아래나 밖에서 작업 중이면 작업 폴더도 함께 보여줍니다. 설정 > 실험 기능에서 켜며, 켜져 있는 동안에만 Console이 git을 읽고 에이전트의 작업 디렉토리를 따라갑니다.
...
#### Changed
- 채팅과 CLI 뷰 사이를 전환하면 응답이 끝날 때까지 기다리지 않고 진행 중인 응답을 중단합니다.
- Repository 패널 개편: 상단 컨텍스트 띠를 없애고 저장소·워크트리 전환기를 사이드바 트리 첫 행으로 옮겼으며, Fetch/Pull/Push/Stash가 기록/변경 세그먼트 탭·기록 필터와 한 작업 줄을 공유하고 패널이 좁아지면 라벨 → 아이콘 → 더 보기 메뉴로 접힙니다.
- Repository 트리가 워크트리·브랜치·원격(원격 이름으로 묶고 호스트 마크 표시)·태그·스태시를 같은 단계의 섹션으로 보여 주며, 현재 브랜치는 체크 표시로, 업스트림이 있는 브랜치는 ahead/behind 수로 표시됩니다.
- Repository 컨트롤이 한 벌의 글리프와 세 층(동사 칩·조용한 아이콘 버튼·세그먼트 탭)만 사용하며, 유니코드 화살표·✕ 글리프, 밑줄 탭, 상시 하단 상태바를 대체했습니다(결과는 잠시 뜨는 토스트로 표시).
- 변경 뷰와 커밋 검사기의 변경 파일 목록이 한 줄 행으로 바뀌었고, 경로가 잘려도 디렉터리는 흐리게·파일명은 보이도록 표시합니다.
- 커밋 검사기의 파일 트리 탭이 파일 탐색기 문법(폴더·파일 아이콘, 가이드 선, 변경 배지)을 쓰고 선택한 파일을 엽니다. 그 커밋에서 바뀐 파일은 diff로, 나머지는 그 커밋 시점의 내용으로 표시됩니다.

#### Fixed
- Quick Launch의 `@`·`/` 덱과 Theater/모델 팝오버가 채팅 뷰의 명령 메뉴처럼 뒤 화면을 흐리게 처리해, 터미널 글자가 선명하게 비치던 문제를 수정했습니다.
- Repository 선택기 행이 컨텍스트 바의 브랜치 스타일을 물려받지 않아 저장소 이름과 브랜치 라벨이 의도한 크기로 표시됩니다.
...
#### Fixed
- Codex(GPT) 게이트웨이 모델의 Claude Code 세션이 다시 답변합니다. Claude Code Artifact 도구의 lookahead·유니코드 속성 패턴처럼 Responses 백엔드가 컴파일하지 못하는 정규식이 도구 JSON Schema에 있으면, 답변이 오기 전에 요청 전체가 400으로 실패했습니다.

### fleet-console

#### Fixed
- Codex(GPT) 게이트웨이 모델로 도는 Operation의 채팅이 다시 동작합니다. Claude Code Artifact 도구의 lookahead·유니코드 속성 패턴처럼 Responses 백엔드가 컴파일하지 못하는 정규식이 도구 JSON Schema에 있으면 요청 전체가 400으로 실패해, 처음 보낸 메시지가 끝내 답변을 받지 못했습니다.
...
#### Added
- Cursor의 Fable 5.1을 게이트웨이 모델로 제공합니다. Low부터 Max까지의 추론 단계와, Cursor API 풀에서 차감되는 Max Mode 1M 변형을 함께 지원합니다.

#### Changed
- 스트리밍 Claude Code 세션에서 GPT-6 Astra가 도구 권한과 순차 호출 제약을 유지하면서 읽기 전용 도구 실행과 독립 작업을 겹쳐 처리하도록 개선했습니다.
- gateway_models에서 로그아웃 상태이거나 노출 모델이 없는 공급자를 제외하고, quota 소비 우선순위를 명시적인 순위 숫자로 제공합니다.

### fleet-console

#### Added
- Settings › AI Gateway에서 켜 두면 Cowork 모델 메뉴에서 GPT-5.6 Luna(Codex)를 고를 수 있습니다.
- Cursor의 Fable 5.1을 게이트웨이 모델로 제공합니다. Low부터 Max까지의 추론 단계와, Cursor API 풀에서 차감되는 Max Mode 1M 변형을 함께 지원합니다.
- 퀘이커 부관단 설정 카드에 부관단 공통 모델·강도 선택기(Claude 별칭과 AI Gateway 모델), 첫 출근 때 한 번 뜨는 소개 말풍선, 마지막으로 말을 건 부관을 여는 Ctrl/Cmd+Shift+Q 단축키가 생겼습니다.
- 부관 대화 카드가 대화 전체를 보여 주며, 여러 줄 입력(Shift+Enter 줄바꿈), 답변 중 「멈추기」, 마지막 답의 「답 복사」·「Quick Launch로 보내기」, 부관이 읽은 페이지의 출처 칩, 답마다 토큰·비용 한 줄, 「대화 지우기」를 갖췄습니다.
- 부관이 입력을 기다리는 Operation을 말풍선으로 알리고, 누르면 그 Operation을 엽니다. 기존 시작·완료 알림과 나란히 섭니다.
- 부관에 포커스를 두고 방향키로 옮기거나 Space로 제자리에 둘 수 있고, 지저귐이 보조 기술에 읽힙니다.

#### Changed
- AI Gateway 모델 로스터를 프로바이더 묶음으로 바꿉니다. 프로바이더는 글리프·켠 모델 수·「우선 소진」 순위 셀렉트·(xAI는) 엔드포인트 스위치를 가진 머리글로 한 번만 서고, 켠 모델은 그 아래에 등급 배지를 앞세운 레코드로 들여 씁니다. 순위 선택지는 켠 모델이 있는 프로바이더까지만이고, 어떤 프로바이더의 마지막 모델을 빼면 우선 소진 순서에서 빠지며 뒤 순번이 당겨집니다. 「모델 추가」 팔레트는 더 크게(기본 720 x 460) 열리고, 오른쪽 변·아래 변·모서리를 끌어 크기를 바꾸며 브라우저별로 그 크기를 기억하고, 항목마다 등급 배지를 앞에 두며, 프로바이더 머리글은 「N종」으로만 수를 적습니다.
- 스트리밍 Claude Code 세션에서 GPT-6 Astra가 도구 권한과 순차 호출 제약을 유지하면서 읽기 전용 도구 실행과 독립 작업을 겹쳐 처리하도록 개선했습니다.
- 패널 캡션·사이드바 칩·그룹 헤더에서 여는 Operation 메뉴가 액센트 9색을 한 줄의 스와치로 보여 주고, 겨눈 색의 이름을 라벨 옆에 읽어 줍니다. 9행 목록은 사라지고, 카드는 산세리프 섹션 라벨·테두리 없는 행·현재 그룹의 체크 표시·중립 그림자를 씁니다.
- 패널의 액센트가 이제 캡션 제목 글자를 물들이고, 제목 앞에 서던 액센트 막대는 사라졌습니다. 사이드바 칩의 스파인과 미니맵 도트는 그대로 액센트를 보여 줍니다.
- 캡션의 더보기 버튼이 창 컨트롤과 같은 14px 격자에 점 세 개를 그리고, 같은 brass hover로 밝아지며, 메뉴가 열린 동안 켜진 상태를 유지합니다.
- Codex Cowork 도크를 수정 스레드로 재구성합니다. 지시마다 턴이 쌓이고, 생각 중·도구 실행·답변 스트리밍을 보여 주는 스텝 원장, "N초 동안 작업함" 접힘, 변경만/전문 토글이 있는 리뷰 도크, 실패·시간 초과·stale 초안·꺼진 모델별 안내가 표시됩니다.
- Cowork 컴포저와 모델 메뉴를 Console 컴포저 문법으로 바꿉니다. 자동으로 늘어나는 입력과 Enter/⇧Enter/Esc 힌트, 공급자 글리프가 붙은 모델 칩과 밴드형 메뉴, 그 옆의 추론 강도 트랙이 표시됩니다.
- Codex 일반 읽기와 확대 표면에 부유 컴포저를 적용하고 입력 시 접히고 비우면 돌아오는 안내와 프리셋을 입력창 안에 배치하며, 선택 댓글은 컴포저 위에서 작성합니다.
- Drydock 제안자를 기계 식별자 대신 읽을 수 있는 출처 표식으로 표시합니다.
- Quick Launch 프롬프트 다듬기가 컴포저 디자인 변경 없이 Operation 멘션을 지원합니다. 후속 메시지 편집은 기존 대화 참조와 행선지·첨부를 보존하며, 전송 전에 사용자가 초안을 명시적으로 적용해야 합니다.
- 부관이 열린 표면을 비켜섭니다. 설정 페인·Quick Launch·대화상자 위에 서지 않고 빠져나오며, 정박한 부관은 표면이 닫히면 제자리로 돌아갑니다.
- Console이 부관 세션을 조용히 거둔 뒤에도 다음 질문이 짧은 알림과 함께 새 세션으로 자동 이어집니다. 대화 오류는 「다시 시도」 버튼이 달린 문장으로 보이고, 조회 행은 답이 끝나면 「웹을 검색함」·「출처 하나를 읽음」으로 정리되며, 긴 멘션 답은 말풍선 안에서 스크롤합니다.
- 플러그인이 어디서나 「퀘이커 부관단」으로 불리고, 부관은 기본적으로 Console 언어로 답합니다.
- Quick Launch의 이미지 첨부 버튼이 컨트롤 행에서 프롬프트 입력 오른쪽 위, 제안 다듬기 버튼 옆으로 옮겨졌고, 멘션·고정 토글은 실행 버튼 옆에 한 무리로 붙습니다.
- Quick Launch 멘션에 대한 퀘이커 부관의 답이 말풍선 안에서 바로 Markdown으로 조판되고, 부관 주변의 남은 공간(창의 60%까지)만큼 자란 뒤에야 스크롤하므로 답이 도착한 자리에서 끝까지 읽힙니다. 말풍선에는 대화 카드를 여는 「이어 묻기」와 「답 복사」가 서고, 기존 「전문 보기」 링크는 사라졌습니다.
- Settings 확대 화면과 버튼을 제거하고 원격 접속 관리를 Settings 패널에서 바로 사용할 수 있도록 변경했습니다.
- 설정의 모든 모델 선택기(AI 확장, 퀘이커 부관단, 압축 시점 미리보기)를 프로바이더 밴드·컨텍스트 창 메타·붙은 강도 세그먼트를 갖춘 하나의 ModelPicker로 통일합니다.
- 설정이 모든 섹션에서 한 제품으로 읽힙니다. 플러그인 카드가 코어 카드와 같은 평행 행·행 제목·도움말·줄마다의 범위 칩을 쓰고, 모델 선택기는 긴 Gateway 모델명을 한 줄로 담는 테두리 있는 한 모양을 공유하며, 모든 켬/끔이 같은 스위치이고, 모든 슬라이더에 −/+ 스테퍼와 숨은 더블클릭 대신 보이는 「기본값」 버튼이 붙습니다.
- 퀘이커 부관단 출근 선택이 다른 설정 선택기와 같은 세그먼트 컨트롤이 되고, 「실험」 배지가 모든 카드에서 같은 문구로 보입니다.
- 설정이 처음부터 넓은 폭으로 열려 AI Gateway 모델 로스터를 포함한 모든 행이 제목 왼쪽·컨트롤 오른쪽의 한 줄 꼴로 보입니다. 좁은 카드에서 세로로 접히던 모습이 기본이 아니게 됩니다.
- 「언어」 칩이 사라지고 표시 언어가 「겉모습」의 첫 카드로 섭니다. 선택된 테마 카드 왼쪽의 금색 띠도 사라집니다.
- 사이드바 칩이 이제 패널 캡션과 같은 방식으로 Operation의 액센트를 칩 제목 글자색으로 보여 주고, 칩 왼쪽에 서던 3px 액센트 막대는 사라졌습니다. 포커스된 칩과 hover한 칩은 액센트를 더 진하게, 나머지는 한 단계 물러난 색으로 씁니다.

#### Fixed
- 캡션 메뉴가 더보기 버튼 오른쪽 변에서 패널 안쪽으로 열려 패널 밖으로 삐져나가지 않고, 위·아래 방향을 실제 높이로 정하며, 낮은 화면에서도 뷰포트 안에 머뭅니다. Enter로 열면 포커스가 메뉴 안으로 들어가고, 좌우 화살표로 액센트 스와치를 오갈 수 있습니다.
- Operation 메뉴의 선택 행에 색상각 보간으로 생기던 초록·분홍 테두리가 사라졌고, 캡션 더보기 말풍선이 열린 메뉴 뒤에 남지 않습니다.
- Quick Launch에서 긴 모델 이름이나 열린 ULTRACODE 게이트가 줄을 채워도 실행 버튼이 컨트롤 행에 남습니다. 실행 버튼이 둘째 줄로 밀리는 대신 Theater·모델 칩 라벨이 줄어듭니다.
...
#### Added
- OpenCode Go에 Grok 4.6, GLM 5.3, 이미지 전달을 지원하는 DeepSeek V4 Flash Vision Exp 등 누락된 현세대 및 독립 라인업 모델 10개를 추가했습니다.

#### Changed
- 단일 출처 벤치마크 순위를 CursorBench와 LiveBench의 정규화된 비교 집합으로 바꾸고, 정확한 effort 프로필을 보존하며 불완전한 측정치는 제외했습니다.

### fleet-console

#### Added
- 설정에 「실험 기능」 카테고리를 추가하고 모두 기본 꺼짐인 옵트인 기능 네 가지를 넣었습니다. 프롬프트 다듬기(Quick Launch 입력창 옆 버튼이 요청을 목표·범위·제약·확인할 질문이 있는 작업 지시문으로 고쳐 쓴 초안을 보여 주며, 적용·버리기·원래대로를 사용자가 고릅니다), 런치 컨텍스트 팩(런치 직전 관련 Wiki 항목과 최근 커밋을 보여 주고 체크한 것만 프롬프트 뒤에 붙입니다), 세션 관찰(Agent Operation마다 관찰 버튼이 생기고 턴이 끝날 때마다 세션 분석가가 범위 이탈·반복 실패·파괴적 시도를 검사해 알립니다), 부관의 Console 읽기(Scuttlebutt 부관이 Operation·Theater·Wiki를 읽기 전용으로 조회합니다)입니다. 모델을 부르는 기능은 스위치 옆에 자기 모델 선택기를 갖습니다(기본은 Claude 별칭, AI Gateway 모델은 그 플러그인이 보탭니다). 관찰 중인 Operation의 캡션 버튼은 마지막 검토 시각과 결과(이상 없음·경고·실패)를 보여 줍니다. 실험 기능 페이지에는 퀘이커 부관단 카드(Console 읽기는 그 안의 한 행)와 Console 포트·원격 접속 카드도 함께 서며, 실험 기능 칩에 마우스를 올리면 이 카테고리의 뜻을 말풍선으로 설명합니다.
- 기존 선택을 유지하면서 OpenCode Go 모델 선택기에 누락된 현세대 및 독립 라인업 모델 10개를 추가했습니다.

#### Changed
- AI Gateway 모델 설정을 로스터 하나와 검색 팔레트 하나로 재편합니다. 켠 모델은 한 목록에 한 줄씩 프로바이더·이름·등급·컨텍스트 창·Fast·강도 사다리·호스트 전용과 함께 서고, 「모델 추가」는 프로바이더와 모델 계열을 함께 검색해 프로바이더별로 묶어 보여주는 팔레트를 열어, 세그먼트 하나에서 컨텍스트 창·Fast 변형을 고른 뒤 한 번에 한 모델을 추가합니다. 팔레트의 프로바이더 머리글마다 그 프로바이더의 설정이 붙습니다. 인라인 API key 칸으로 로그인하거나 로그아웃하고, 「우선 소진」 순위 셀렉트와 xAI 엔드포인트 스위치를 씁니다. 순위가 있는 프로바이더는 로스터 줄에 표시되고, 로그인되지 않은 프로바이더의 모델은 로그인하기 전까지 숨겨집니다. 별도의 「AI Gateway용 API key」 카드는 사라집니다.
- Repository를 History와 Changes 중심으로 정리하고, 통합 저장소·워크트리 선택기, 범위가 명확한 참조 검색, Amend 진입을 보존한 빈 상태, 체크아웃별 메모리 커밋 초안, 고정 작업 결과 행을 제공합니다.
- 사이드바의 그룹/상태 세그먼트를 Theater마다 대기·실행 중·유휴로 묶는 「상태별로 보기」 토글(Alt+S) 하나로 바꾸고, 0건 상태 섹션과 선반은 표시하지 않습니다. 「레일로 좁히기」 토글로 사이드바를 64px 레일로 좁힐 수 있으며 Cruise/Tactical은 Theater별로, War Room은 상태별로 묶이고(War Room은 레일이 기본) 마우스를 올리면 펼쳐집니다. 레일 도구는 Theater 범위와 Fleet 범위로 나뉘며 설정 기어는 열 끝으로 내려갑니다. 커맨드 밴드 트레이 폭을 고정해 모드를 오갈 때 모드 스위치가 흔들리지 않습니다.
- Skills 탐색을 차분한 목록과 통합 상세·설치 시트로 정돈하고, 적용 범위와 에이전트 선택, 키보드 탐색, 검색·목록 불러오기 실패 안내를 명확히 했습니다.
...
#### Changed
- fleet gateway의 OpenCode Go 모델 선택기에서 Muse-Spark-1.2-Contributor 대신 Muse-Spark-1.3-Contributor를 제공하며, 1M 컨텍스트 창과 지원하는 추론 강도 단을 유지합니다.

### fleet-console

#### Added
- 파일 트리에서 파일에 Space를 누르면 문서를 열지 않고 행 옆 카드에서 첫 줄·이미지·크기를 훑어볼 수 있습니다. Enter는 열기, Escape나 Space는 닫기, 방향키는 다음 파일로 훑기를 옮깁니다.
- 깊은 폴더를 스크롤하는 동안 열린 조상 폴더가 파일 트리 상단에 고정되어 남고, 고정된 폴더를 누르면 그 행으로 돌아갑니다.
- 파일 트리 행에 마우스를 올리면 훑어보기·상대 경로 복사·파일 관리자에서 보기 동작이 나타나고, 복사 확인은 토스트 대신 그 자리에서 표시됩니다.
- 문서 헤더의 경로 조각을 누르면 그 폴더의 파일 목록이 열려 바로 이동할 수 있고, 헤더의 복사 버튼이 상대 경로(Alt+클릭은 절대 경로)를 복사하며 확인은 제자리에 표시됩니다.
- 좌측 사이드바의 불투명도와 유리 흐림을 설정에서 직접 정할 수 있습니다. 우측 사이드바 불투명도 손잡이 바로 아래에 섭니다.

#### Changed
- Cruise 캔버스 축소가 함대 지도에서 멈춥니다. 모든 Operation이 점으로 잦아든 뒤의 추가 축소는 지도를 그대로 둔 채 바다만 옅게 만들어 축소의 끝을 손으로 알 수 없었는데, 이제 휠이 그 고도에서 멈추고 확대는 그 자리에서 그대로 돌아갑니다.
- 파일 트리 헤더가 테두리 없는 필터 한 줄로 쉬며, 입력 중에만 파일/내용 범위가 필드 안에 나타나고 정렬·숨김 파일·새로고침은 메뉴 하나로 옮겨졌으며 검색 상태는 한 줄로 합쳐졌습니다.
- 파일 트리의 Git 상태가 M/U/D 글자 대신 파일명 색조와 행당 점 하나로 표시되고, 폴더는 안쪽의 가장 강한 변경을 점 하나로 보여주며 행 밀도가 조금 높아졌습니다.
- 열린 문서가 밑줄 탭으로 표시되어 닫기 버튼은 마우스를 올릴 때만 나타나고, 디스크에서 바뀐 문서는 닫기 자리에 점이 표시되어 클릭하면 다시 읽으며, 가운데 클릭으로 탭을 닫고 넘친 탭은 "+N" 대신 목록으로 열립니다.
- 파일 트리가 불러오는 동안 골격 행을, 폴더를 펼치는 동안 지연 링을, 빈 폴더에는 숨김 파일 바로가기가 있는 안내를, 로드 실패에는 다시 시도 카드를 보여주고, 트리와 코드 뷰어의 스크롤 가장자리가 옅어집니다.
- Fleet Console의 OpenCode Go 모델 선별에서 Muse-Spark-1.2-Contributor 대신 Muse-Spark-1.3-Contributor를 제공하며, 1M 컨텍스트 창과 지원하는 추론 강도 단을 유지합니다.
...
#### Fixed
- Antigravity Gemini 모델 사용 시 튜플 형태의 도구 스키마를 허용합니다.

### fleet-console

#### Added
- AI Gateway 모델 선택기에 Antigravity Gemini 3.8 Flash를 추가합니다.
- Cruise 캔버스를 판독 한계 아래로 축소하면 패널 자리에 함대 지도가 섭니다. Theater마다 구역이, Operation마다 상태 색 점이 서고 점을 누르면 그 Operation으로 내려가며, Theater 표석을 누르면 그 Theater가 올라오고, 다시 확대하면 겨누던 점의 패널로 내려갑니다.

#### Changed
- 채팅 턴마다 살아 있는 줄을 하나만 둡니다. 모델이 생각하는 공백에는 별도 상자 대신 집계 줄 꼬리의 "생각 중..." 점이 하나부터 셋까지 순환하고, 링과 물결은 꼬리가 무엇인가를 말할 때만 돌며, 흐르는 응답에는 캐럿과 마지막 줄 옅어짐이 붙습니다.
- 채팅 원장의 문자 글리프(집계 계열·잡 앵커·작업 면·세션 분석가 시길)를 무게가 균일한 모노라인 아이콘 세트로 교체합니다.
- 채팅 뷰의 백그라운드 작업이 입력창 위 선반에 자리합니다. 스피너와 상태 문구 전체가 함께 강조되며 작업 목록을 대화 위 시트로 여닫고, Esc나 대화 클릭으로 다시 접습니다. 선반이 입력창 글리프와 나란히 서던 작업 면을 대신합니다.
- War Room 덱 밀도는 1.0×~2.0× 사이만 지원합니다. 0.4× 지도 밀도는 퇴역하고 그 역할은 Cruise 함대 지도가 맡습니다.
- 채팅 작업 선반을 입력창에 맞춰 정렬하고 짧은 지시의 작성 영역을 더 간결하게 표시합니다.

#### Fixed
- 세션 분석가 아티팩트 안의 한글을 OS 폴백 대신 번들된 콘솔 서체(본문은 Pretendard, 라벨·코드는 코딩 서체)로 렌더합니다.
- 고정폭 UI 텍스트(세션 분석가 캡션·칩·상태 줄)의 한글을 OS 폴백 대신 번들 코딩 서체로 렌더합니다.
- 채팅 모드에서 빠른 Shell 작업이 실행부터 완료까지 하나의 연속된 행으로 읽히도록 개선했습니다.
- 진행 중·실패 스텝 테두리를 oklab으로 섞어 Whites 테마에서 상태색 대신 녹회색 테두리가 보이던 문제를 고칩니다.
...
#### Added
- 접힌 패널은 화면 가장자리에 얇은 brass 필라멘트를 남깁니다. 필라멘트에 마우스를 올리면 캔버스 배치를 건드리지 않고 패널이 오버레이로 살짝 나타나고, 클릭하거나 미리보기 안의 고정 버튼을 누르면 다시 열린 상태로 고정됩니다.

#### Changed
- 세션 분석가 아티팩트가 콘솔의 디자인 언어를 그대로 잇습니다. 문서가 패널면 위에 앉고, 카드가 위로 들리며, 조용한 섹션 키커·콘솔 서체·중앙 읽기 컬럼·호스트 컴포넌트 세트가 네 테마 전부에서 일관된 아티팩트를 보장합니다.
- 세션 분석가 채팅이 채팅뷰의 원장 문법으로 말합니다. 스트리밍 중에는 과정 문장이 구간으로 쌓이는 동안 도구는 한 줄이 제자리에서 갱신되고, 끝난 턴은 전체 스텝을 품은 한 줄로 접히며, 확정 답변은 중간 서술과 병합되지 않고 응답 구분선 아래 홀로 섭니다.
- 분석가 입력창이 채팅뷰와 같은 조립을 씁니다. 입력 위층과 좌표 레일 아래층이 첫 질문 뒤에도 유지되고, 초기 중앙 입력창은 채팅뷰와 같은 모션으로 스트리밍 시작과 함께 하단에 내려앉습니다.
- Cruise / Tactical / War Room 모드 스위치가 커맨드 밴드 정중앙에 자리하며, 중앙을 유지할 수 없는 좁은 창에서만 좌측 클러스터로 물러납니다.
- 세션 검색이 커맨드 밴드 중앙, 모드 스위치 오른쪽으로 이동해 어느 화면에서든 하나의 전역 진입구로 상주합니다.
- 사이드바와 Activity Rail 접기가 각 패널 자신의 컨트롤로 옮겨졌습니다. 패널 안의 접기 버튼이 패널을 접고, Cmd+B / Cmd+Alt+B 단축키는 같은 상태를 그대로 토글합니다.
- 저장소 컨트롤이 쉬는 상태에서도 채움과 테두리의 몸을 갖습니다. 주 동사·인스펙터 세그먼트·뷰 토글·깊이 스테퍼·행 액션이 hover 전까지 맨글자로 서 있지 않고, 입력 상자는 표면보다 어두운 우물로 파이는 대신 표면 위로 떠오릅니다.
- 저장소 패널의 밴드들이 한 면으로 통일됩니다. identity 캡·탭·툴바·트리 열·인스펙터 셸프가 제각각 어두운 띠 대신 하나의 은은한 워시를 공유하고, 파일 상태 글자는 틴트 칩, ref 뱃지는 캡슐로 부드러워집니다. 선택 표시는 익숙한 중립 워시와 측면 스파인 그대로입니다.
- 모든 설정의 설명을 항목 제목 옆 '?' 글리프 뒤로 옮깁니다. hover로 미리 보고, 클릭·탭으로 고정해 읽을 수 있으며, Esc나 바깥 클릭으로 닫힙니다. 상태 경고·오류·실시간 판독은 인라인에 그대로 남습니다.

#### Fixed
- 자동 슬롯이 없는 표면의 강도 트랙이 최저 단에서 손잡이 뒤로 brass 조각을 남기지 않아, 최저 설정도 사다리의 다른 단처럼 깨끗하게 읽힙니다.
- 긴 분석이 2분에서 죽지 않습니다. 고정 120초 응답 상한이 이벤트 수신마다 재무장되는 무활동 감시로 바뀌어, 아티팩트 저작 같은 긴 턴이 이벤트를 보내는 한 계속 진행됩니다.
- 분석가 채팅 응답의 근거 인용이 모델이 아티팩트 인용 표기를 쓰더라도 원문 마크업으로 노출되지 않고, 어디서든 같은 증거 칩으로 렌더됩니다.

#### Removed
- 커맨드 밴드 중앙의 Theater·Operation 브레드크럼 전환기를 제거했습니다. 전환과 이름 변경은 사이드바가 단일 창구로 유지됩니다.
- 커맨드 밴드의 좌·우 패널 접기 버튼 2개를 제거했습니다. 밴드에는 정체·캔버스 모드·검색·시스템 컨트롤만 남습니다.
- 사이드바 카드 헤더와 인라인 Operation 필터를 걷습니다. Theater 라벨·개수·헤더의 새 Theater 버튼이 사라지고 목록은 그룹/상태 전환에서 시작합니다. 새 Theater는 목록 맨 아래 행과 커맨드 밴드에서 그대로 열 수 있습니다.
...
#### Added
- 채팅뷰 세션에서도 Artifact를 게시할 수 있고, 컴포저 덱에 artifact 스킬이 함께 섭니다.
- 우클릭 메뉴에서 Claude Operation을 채팅뷰로 곧장 실행합니다. 모델 행마다 시작 표면 표식이 서고, 눌러서 또는 왼쪽 방향키로 바꿉니다. 메뉴는 그 표면을 다음에 열릴 때까지 기억하며, 채팅으로 시작할 수 없는 실행 종류의 행에는 표식이 서지 않습니다. 이 기억은 Quick Launch와 따로 삽니다.

#### Changed
- Session Analyst 아티팩트가 Console 테마와 조판 바닥 위에서 렌더되어, 제목·표·코드·근거 인용이 제품의 일부로 읽힙니다.
- 채팅 입력창이 선택한 읽기 폭을 따라가고, 컴포저 행의 새 컨트롤로 패널 전폭까지 넓힐 수 있습니다.
- Activity Rail 패널 폭을 도구별로 기억합니다. 한 패널을 넓혀도 나머지 패널이 함께 넓어지지 않고, 한 번도 조절하지 않은 도구는 계속 그 패널이 선언한 폭으로 열립니다.
- 설정 패널이 기본 상태에서 테마 격자를 2열로 세울 만큼 넓게 열립니다.

#### Fixed
- 아티팩트를 내려받으면 테마와 스타일을 그대로 지닌 자립 문서가 저장됩니다. 소스 복사는 분석가가 쓴 원본을 그대로 줍니다.
- 느린 Windows 시작이 준비 완료까지 기다리며, 시작 실패 시 뒤에 Console 프로세스를 남기지 않고 정리합니다.
- 레일 크기 조절 손잡이를 더블클릭하면 지금 열린 패널의 폭만 초기화하고, 다른 도구가 기억한 폭은 그대로 둡니다.
- 사이드바의 활성 Theater를 brass 스파인과 채워진 이니셜 배지, 밝은 이름으로 표시해 다른 Theater에 마우스를 올려도 작업 중인 Theater보다 강조되지 않습니다.
...
#### Added
- Claude Code의 Codex Gateway 모델에서 자동 및 수동 압축, 영속 재개, 안전한 일반 텍스트 폴백을 포함한 OpenAI 네이티브 압축을 사용합니다.

#### Changed
- `fleet`는 이제 승인 게이트를 세운 auto 모드로 Claude Code를 실행합니다. 이전처럼 게이트 없이 열려면 Fleet Console 설정의 하네스 항목에서 "승인 프롬프트 건너뛰기"를 켜세요.

### fleet-console

#### Added
- Claude Code의 Codex Gateway 작전에서 자동 및 수동 압축, 영속 재개, 안전한 일반 텍스트 폴백을 포함한 OpenAI 네이티브 압축을 사용합니다.
- 사이드바에 Theater 수와 새 Theater 동작, 인라인 Operation 제목 필터를 갖춘 자기 완결 카드 헤더를 추가합니다.
- 설정에 하네스 항목이 생겨 에이전트를 어떻게 실행하는지가 한곳에 모입니다. 승인 옵트인과 시스템 프롬프트를 담은 Claude Code 카드, 하네스 공통 세션 설정, 에이전트 CLI 실행 파일 목록이 함께 놓입니다.
- "승인 프롬프트 건너뛰기"는 기본이 꺼짐인 새 옵트인입니다. 꺼 두면 세션이 Claude Code auto 모드로 돕니다. 승인 게이트가 그대로 서고, 판정기가 대부분의 승인을 대신 처리하며, 확인이 필요하면 터미널에서 묻고, 판단이 서지 않는 것은 막습니다. 카드가 적용 범위를 함께 말합니다: 터미널과 `fleet` 런처에 적용되고, 자체 승인 화면이 없는 채팅은 언제나 바이패스로 실행됩니다.

#### Changed
- Cruise, Tactical, War Room 모두에서 작전 해도를 뷰포트 전면으로 확장하고, 사이드바와 Activity Rail을 그 위에 뜬 유리 카드로 바꾸며, 모든 모드 배치와 맞춤, 발사, 미니맵을 가려지지 않은 아레나 기준으로 계산합니다.
- 커맨드 밴드를 하나의 연속된 판으로 통일합니다. 사이드바 상단 캡 이음선을 퇴역시키고 Cruise, Tactical, War Room 스위치가 고정 위치의 좌측 클러스터에 합류합니다.
- Activity Rail 카드에는 패널이 한 번에 하나만 상주합니다. 다른 패널을 열면 현재 패널이 교체되고, 패널을 열어도 해도를 밀어내지 않으며, push와 overlay 이원은 부유 카드로 단일화되어 퇴역합니다.
- 새 Claude Code 세션은 더 이상 기본으로 승인 게이트를 건너뛰지 않고 auto 모드로 열립니다. 이전 동작이 필요하면 설정의 하네스 항목에서 옵트인을 켜세요.
- 터미널 설정 항목에는 이제 그리는 것만 남습니다. 터미널 글꼴, 채팅 읽기 폭, 그리기입니다. Claude Code 시스템 프롬프트와 유휴 에이전트 세션 설정은 하네스로 옮겼고, 에이전트 CLI 실행 파일 목록도 AI Gateway에서 하네스로 옮겼습니다.
- File Explorer 뷰어가 레일의 독립된 열로 서고, 호스트 캡션이 열린 문서 이름과 이력·다시 읽기·줄바꿈·확대를 함께 싣습니다.
- 레일의 열은 하나의 분할선을 씁니다 — 목록과 문서 사이 경계를 끌거나 화살표로 옮길 수 있고, 표면마다 그 자리를 기억합니다.
- Codex 문서가 레일의 독립된 열로 서고, 이력과 확대가 그 캡션에 놓입니다.
- Repository 레일 아이콘이 Shell처럼 작업면을 캔버스에 엽니다 — 배치는 그대로이고 폭만 훨씬 넓어집니다. 다시 누르면 치워집니다.
- 설정이 전용 페이지에서 활동 레일 톱니 뒤의 설정 페인으로 옮겨졌습니다. 섹션은 줄바꿈 칩으로 서고, 검색은 모든 설정을 덮으며 커맨드 팔레트까지 닿고, 원격 접속 같은 밀도 높은 관리면은 확대 표면에서 열리며, 겉모습을 고치는 동안 콘솔이 옆에 살아 있습니다.
- 레일 톱니 메뉴가 해체되었습니다. 플로팅과 불투명도는 겉모습 섹션에 살고, 패널 분할선을 더블클릭하면 기억된 패널 폭이 초기화됩니다.
- 레일 패널 불투명도 조절이 "우측 사이드바 불투명도"라는 이름으로 테마 카드의 비포커스 패널 흐리기 바로 아래로 옮겨졌고, 전용 레일 패널 카드는 은퇴했습니다.

#### Fixed
- 리퀴드 글래스를 켰을 때 좌측 사이드바에 희미한 가짜 격자 모자이크가 비치던 결함을 고쳤습니다. 약한 유리 블러가 뒤 해도 위브를 덜 뭉개 그 잔물결이 어두운 틴트 위에서 계단으로 남던 것으로, 이제 카드는 우측 사이드바 패널과 같은 강한 블러를 써 유리 질감을 그대로 유지한 채 깨끗하게 섭니다.

#### Removed
- 데스크톱 전용 설정 페이지와 그 안의 겉모습 미리보기 모형, 커맨드 밴드의 설정 버튼이 은퇴했습니다. 옛 /settings 링크는 새 페인으로 착지하고, 폰의 설정 화면은 그대로 유지됩니다.
...
#### Changed
- Activity Rail 설정을 아이콘 열 최상단의 톱니로 옮기고 디바이더로 패널 탭과 갈랐습니다. 톱니를 누르면 Map 위에 띄우기, 패널 불투명도, 패널 폭 초기화, 패널 닫기가 담긴 메뉴가 열립니다.
- 전체 화면 터미널 애플리케이션이 대체 화면을 사용하는 동안 자체 스크롤을 사용하고 스크롤바와 셀 미만의 격자 여백을 회수하며 종료하면 일반 터미널 기록으로 복귀합니다.
- Agent Operation과 전역 Shell이 안쪽 여백 없이 터미널 본문 전체를 사용합니다.
- 호환 가능한 터미널 메타데이터를 유지하면서 Shell 및 Agent 터미널에서 실행되는 프로그램에 24비트 색상 지원을 알리고, 다크 리퀴드 글래스 터미널 패널이 스모크 반투명 표면을 통해 캔버스를 비추도록 개선합니다.

#### Fixed
- 세션에 포커스가 있는 동안 Session Analyst 패널이 또렷하게 남습니다. 방금 연 패널이 곁의 패널과 함께 물러나지 않습니다.
- 새 소식 언어 선택기가 Console 표시 언어를 덮어쓰지 않고 릴리스 노트에만 적용되도록 수정합니다.
- 사이드바 표면을 닫았다가 다시 열어도 전역 Shell 세션과 터미널 내용을 보존합니다.

#### Removed
- 레일 패널이 본문 상단을 지나는 포인터에 헤더를 띄우지 않습니다. 첫 줄의 패널 컨트롤이 가려지지 않고 본문이 슬롯 전체를 씁니다.
