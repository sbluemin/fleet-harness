...
- **Dependency direction:** `fleet-cli` -> `fleet-carriers` -> `core-infra`, with `fleet-console` as the local Console Service, `fleet-desktop` -> `fleet-console` for the optional native shell, and `core-agent` consumed as a generic leaf.
