...
- **Dependency direction:** `fleet-cli` -> `fleet-carriers` -> `core-infra`, with `fleet-console` as the local observation backend and `core-agent` consumed as a generic leaf.
