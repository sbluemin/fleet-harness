...
- Fleet Plan authoring and mutation through host-only `plan_write`, with `plan-operations` loaded before the first call in a session; Kirov may optionally audit only an existing host-authored PlanRef.
