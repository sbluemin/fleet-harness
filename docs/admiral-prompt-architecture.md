...
edit_policy: |
  ...
  for how the system prompt and live Fleet state flow through fleet-agent,
...
tag prefix. The Admiral system prompt is assembled inside `fleet-agent`, and
...
`buildSystemPrompt()` in `packages/fleet-agent/src/admiral/prompts.ts` assembles
...
`packages/fleet-agent/src/grand-fleet/prompts.ts` builds the Fleet ACP prompt.
...
`packages/fleet-agent/src/runtime/runtime.ts` is the lifecycle boot entry point.
