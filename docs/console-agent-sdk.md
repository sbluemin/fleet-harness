...
- `ctx.host.consoleControl`(있는 호스트에서)은 `console_launch`·`console_send`와 같은 서버 경로로 Operation을 시작하거나 그 입력창에 메시지를 넣는다. 호출자는 그 플러그인 자신이며 시트를 거치지 않는다. `ctx.host.operations.groups`는 사이드바의 Operation 그룹을 플러그인 목록으로 연다. Operation의 그룹이 바뀌면(사람의 이동·Console Use·플러그인 `patch({ groupId })`·그룹 삭제) 서버 안 채널 `operation:grouped`가 한 번 나므로, 그룹을 목록으로 쓰는 플러그인은 이것을 구독해 연결 항목을 따라 옮긴다.
