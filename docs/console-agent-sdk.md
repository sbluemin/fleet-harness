...
- `consoleRead`는 Console 기본 읽기 도구의 명시적 요청이다. `enabled`는 호출마다 평가하며, `snapshot`은 제품의 메시지 시점 활동을 제공한다. 전역 Admiral 도구는 자동 연결하지 않는다.
...
- Scuttlebutt: 대화 연속성, 웹 도구, 선택적인 Console·Wiki 읽기. 페르소나·제품 세션·SSE 표현은 플러그인 소유.
