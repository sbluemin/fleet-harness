...
각 Carrier의 CLI 백엔드, 모델, 추론 강도, Task Force 구성을 하나의 시각적 로스터에서 선택합니다. 기본 제공되는 4개의 전문 Carrier는 전략 판단과 선택적 Plan 보증, 직접·Plan 기반 구현, QA·보안, 로컬·원격 통합 코드베이스 인텔리전스를 담당합니다. 제독(호스트)이 TaskRef 완료 표시를 포함한 모든 Fleet Plan 상태를 직접 작성·변경하며, 완료 표시는 Artifact Inspection과 Lane QA를 확인한 뒤에만 수행합니다. Nimitz는 호스트가 작성한 정확한 PlanRef를 변경 없이 선택적으로 감사하고, Genesis는 호스트가 작성해 전달한 동일 Lane TaskRef를 실행합니다. 문서화와 Fleet Wiki 갱신도 Carrier에 위임하지 않고 제독(호스트)이 직접 수행합니다.
