...
`fleet console`을 권장하고, 과도기 `fleet-console`도 `status`, `restart`, `stop`을 지원합니다. 모든 실행은 내 머신 안에서 이루어집니다. 서버는 기본적으로 루프백에 바인딩되고, 브라우저에는 MCP·공급자 토큰이 전달되지 않습니다. 원격 접속은 실험 기능이며 켜기 전까지 꺼져 있고, 켜면 직접 고른 인터페이스에 TLS로 콘솔을 엽니다.
