...
const TRAMPOLINE_ENV = "FLEET_GATEWAY_TRAMPOLINED";
...
function maybeTrampolineToLocalCli(): void {
  ...
  process.stderr.write(`${cyan}fleet-gateway: redirecting to ${localCli}${reset}\n`);
  ...
  if (result.error) {
    process.stderr.write(`fleet-gateway: trampoline failed: ${result.error.message}\n`);
    process.exit(1);
  }
  if (result.signal) {
