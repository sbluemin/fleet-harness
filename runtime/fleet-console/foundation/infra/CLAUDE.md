...
- `createStoreCarryOver` owns moving a value from a previous location exactly once, and removes that file once the value is safely in its new home. Completion is the destination's existence, never emptiness, so a user who cleared a setting does not get the old value resurrected; while a previous file cannot be read the caller must refuse to write, because the first write to the destination ends the migration and strands whatever was not read; and a source that resolves to the destination is dropped, or the step that removes the old file deletes the live one.
