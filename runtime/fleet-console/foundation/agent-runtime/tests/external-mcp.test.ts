...
describe("resolveBuiltinExternalMcpServers", () => {
  ...
  it("host가 주입한 예약 ID는 throw", () => {
    expect(() => resolveBuiltinExternalMcpServers(["host"], { reservedIds: ["host"] })).toThrow(/host/);
  });
  ...
  it("내부 MCP 토큰 재사용과 external 누수를 거부한다", () => {
    expect(() => assertInternalMcpTokensNotShared([], [
      { serverName: "fleet", token: "same-token" },
    ...
    expect(() => assertInternalMcpTokensNotShared([
      {
        ...
        name: "grep_app",
        url: "https://mcp.grep.app",
        headers: [{ name: "Authorization", value: "Bearer internal-token" }],
      },
    ], [
      { serverName: "host", token: "internal-token" },
    ])).toThrow(/leaked/);
  });
});
