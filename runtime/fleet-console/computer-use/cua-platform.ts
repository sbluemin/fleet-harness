...
export function createCuaComputerUsePlatform(directory: string, runtime: ComputerUseRuntimeDependencies): ComputerUsePlatform {
  ...
  return {
    ...
    actionSchemas: tools => Object.fromEntries([...tools].filter(([name]) => name !== "paste").map(([name, tool]) => {
      ...
      return [name, { ...tool.inputSchema, properties, required, description: "Cua Driver background delivery only. No automatic foreground fallback or follow-up observation. Verify with an explicit computer_state after the action. " + (tool.description ?? "") }];
    })),
    hasFullObservation: value => !value.isError && cuaData(value).cuaObservation === true,
    hasActionObservation: () => false,
    interactionHints: () => ["Cua Driver targets one exact window and sends background input. A returned action is not task success and does not provide a new snapshot. Request computer_state explicitly to verify. Only actions in actionSchemas are supported; no automatic substitute, foreground escalation, or retry."],
