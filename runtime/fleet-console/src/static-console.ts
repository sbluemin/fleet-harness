...
export function tryServeStaticConsole(req: http.IncomingMessage, res: http.ServerResponse, pathname: string): boolean {
  ...
  try {
    ...
    if (pathname.startsWith("/console/")) {
      return serveFallbackIndex(req, res);
    }
    res.writeHead(404, withSecurityHeaders({ "Content-Type": "application/json" }));
    res.end(JSON.stringify({ error: "Not found" }));
    return true;
...
function serveFallbackIndex(req: http.IncomingMessage, res: http.ServerResponse): boolean {
  try {
    const data = fs.readFileSync(path.join(CONSOLE_ROOT, "index.html"));
