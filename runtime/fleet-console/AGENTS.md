...
- A remote listener is the opposite default: every request it accepts requires a session opened through a single-use grant, decided once before routing, and the only exceptions are the join endpoint and the static notice that explains an access link. Sessions are bound to the listener that issued them and never cross between loopback and remote.
