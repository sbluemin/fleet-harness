...
- Codex is a **Right Rail built-in panel** (after ALERTS; `BUILT_IN_RAIL_PANELS = [alertsPanel, codexPanel]`). A single shared mount host (`codex-host.ts`) relocates the Vanilla singleton into the Rail panel container with `appendChild`; `destroy()`+remount is never called. The presentation mode is always `side` — `codex-host.ts:mountCodexInto` calls `setCodexPresentationMode("side")` before each mount. Workspace selection is driven by Theater state (`ctx.theaterId` from `RailPanelContext`); the left workspace switcher is removed. Mount paths: Theater auto-registration (`server.ts` `codex.registerWorkspace(cwd)`) and restart restoration (`server.ts` `restoreCodexWorkspaces`). The admin workspace registration endpoint, bearer-token surface, and standalone `fleet-wiki` binary are all removed. The Codex layout is container-responsive — it sheds the rail (≤900px), then the nav (≤560px), and at ≤380px renders a single-column body-only view for the 312px Rail slot. The panel header carries manual collapse toggles for the left (Nav) and right (ToC/Manifest) panes — persisted at `fleet-console.codex.nav-collapsed`/`fleet-console.codex.rail-collapsed` and applied via `app-shell--nav-collapsed`/`app-shell--rail-collapsed` grid classes. The pane-collapse state is owned by the Vanilla Codex module and exposed to the React surface only through the `codex-host.ts` boundary.
- The Codex reading surface uses a single `app-shell` grid for every route. The reading measure (78ch at 17px / 1.78) is owned by `.markdown-body` and its reading-wrapper siblings (`.document-header`, `.related-list`) — never the shell grid; the shell max-width is 1680px and the rail track is `clamp(272px, 20vw, 320px)`. Shell modifiers are limited to `app-shell--wide` (browse routes home/search/drydock/conflicts — rail removed) and `app-shell--nav-collapsed`/`app-shell--rail-collapsed` (manual pane collapse). The deprecated `app-shell--raw` modifier is removed — raw source content is now embedded inline via `GET /api/entry/:id?include=raw` and never served as a separate shell route. Modifiers only set grid columns/areas (single-class or `:where()` specificity) so the 900px/560px container queries still win by cascade, and drydock-detail keeps its own internal `1fr + 300px` layout. The reading rail renders the ToC before the Manifest (the Manifest is an open-by-default `<details>`), the ToC has an `IntersectionObserver` scroll-spy that cleans up on every route change, and below 760px the rail is replaced by a document-level ToC drawer (a focus-trapped `role="dialog"`). Reading entries carry a breadcrumb and drop a duplicate title only when the first markdown H1 exactly normalizes to the entry title. Codex typography is tokenized through `--font-size-*`; brass = active/location and aurora = live/link semantics, glass surfaces, self-hosted Fraunces/Manrope/JetBrains Mono, and `prefers-reduced-motion` short-circuits are unchanged.
- Preserve Maritime Codex UX: reading flow (raw source embedded via `?include=raw` — no separate raw viewer route), Drydock, conflicts, command palette, Manifest/ToC rails, copy-context actions, diagram lightbox, brass/aurora roles, and self-hosted fonts. The deprecated index/log view routes and standalone raw viewer shell are removed.
