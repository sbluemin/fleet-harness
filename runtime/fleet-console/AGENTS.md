...
- The Codex reading surface is a centered modal **Reading sheet** (`CodexReadingSheet`, `createPortal` to `document.body`) over the Operations canvas — not an `app-shell` grid route. The reading measure (72ch in the sheet) is owned by `.markdown-body` and its reading-wrapper siblings (`.document-header`, `.related-list`), never a shell grid. The sheet is `role="dialog"` + `aria-modal="true"`, focus-trapped, closes on Esc/scrim/✕ with focus restored to the triggering Navigator entry, toggles width 760↔960px via `⌘\`/`Ctrl+\`, marks the background canvas/SideBar inert via `body[data-codex-reading]`, and uses a one-shot `codex-rise` entry with a `prefers-reduced-motion` short-circuit. The sheet carries a 200px ToC rail with an `IntersectionObserver` scroll-spy (brass = active section). Reading entries carry a breadcrumb. Codex typography is tokenized through `--font-size-*`; brass = active/location and aurora = live/link semantics, glass surfaces, self-hosted Fraunces/Manrope/JetBrains Mono, and `prefers-reduced-motion` short-circuits are unchanged. The deprecated `app-shell`/`app-shell--wide`/`app-shell--raw` grid, container queries, document-level ToC drawer, and Manifest panel are removed.
- Preserve Maritime Codex reading aesthetic: the markdown render pipeline (`renderMarkdown` — marked + DOMPurify + hljs + wiki links; `diagrams.ts` Mermaid `securityLevel: "strict"`) and the `.markdown-body`/`.document-header`/`.related-list`/`.code-block` styling are preserved verbatim and reused inside the Reading sheet. Surfaces: Navigator (search + entry list + Drydock + conflicts) and Reading sheet (markdown body + ToC scroll-spy + copy-context actions + diagram lightbox), with brass/aurora roles and self-hosted fonts. The standalone raw viewer route, index/log views, command palette, Manifest panel, and document-level ToC drawer are removed; raw source is embedded inline via `GET /api/entry/:id?include=raw`.
