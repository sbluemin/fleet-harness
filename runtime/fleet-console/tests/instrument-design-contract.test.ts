...
describe("Instrument core design contract", () => {
  ...
  it("keeps the v4 navigation, Theater, map, CLI, and rail visual producers", () => {
    ...
    expect(operationFrame).toContain("!active && rename.renaming");
    expect(operationFrame).toContain("!active ? (");
    ...
    expect(components).toContain("color: color-mix(in oklch, var(--ink-pearl) 72%, transparent);");
    expect(components).toContain("background: color-mix(in oklch, var(--ink-deep) 76%, transparent);");
    ...
    expect(components).toContain("right: var(--space-2);");
    expect(components).toContain(".canvas-operation:not(.is-active) .canvas-operation-titlebar {");
