...
async function mount(options: MountOptions): Promise<{
  ...
  const ctx = {
    ...
    host: {
      operations: {
        get: () => ({ id: "op-1", type: "shell", pluginId: null, theaterId: "theater-1", payload: {} }),
        list: () => [],
      },
      paths: {
  ...
  return {
    ...
    async call(body) {
      requestBody = body;
      await handler({
        req: { method: "POST", url: "/api/v1/shell/ticket", headers: {} } as http.IncomingMessage,
        res: {} as http.ServerResponse,
        pathname: "/api/v1/shell/ticket",
