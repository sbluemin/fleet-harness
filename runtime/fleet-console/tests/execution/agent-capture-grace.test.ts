...
async function createHarness(body: Record<string, unknown>) {
  ...
  return {
    ...
    deleteSession: async (sessionId: string) => {
      ...
      await route({
        req: { method: "DELETE", url: `/api/v1/agent/sessions/${sessionId}` } as http.IncomingMessage,
        res: {} as http.ServerResponse,
        pathname: `/api/v1/agent/sessions/${sessionId}`,
      });
    },
    ...
    resumeSession: async (sessionId: string) => {
      ...
      await route({
        req: { method: "POST", url: `/api/v1/agent/sessions/${sessionId}/resume` } as http.IncomingMessage,
        res,
        pathname: `/api/v1/agent/sessions/${sessionId}/resume`,
      });
    },
    turnSession: async (sessionId: string) => {
      if (!route) throw new Error("Agent route was not registered");
      await route({
        req: { method: "POST", url: `/api/v1/agent/sessions/${sessionId}/turn` } as http.IncomingMessage,
        res: {} as http.ServerResponse,
        pathname: `/api/v1/agent/sessions/${sessionId}/turn`,
      });
    },
    backgroundSession: async (sessionId: string) => {
      if (!route) throw new Error("Agent route was not registered");
      await route({
        req: { method: "POST", url: `/api/v1/agent/sessions/${sessionId}/background` } as http.IncomingMessage,
        res: {} as http.ServerResponse,
        pathname: `/api/v1/agent/sessions/${sessionId}/background`,
      });
    },
  };
}
