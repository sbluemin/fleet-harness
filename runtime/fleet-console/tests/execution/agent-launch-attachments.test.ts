...
} from "../../features/execution/host/agent/launch-attachments.js";
...
async function createHarness(options: { readonly attachError?: Error } = {}) {
  ...
  return {
    ...
    postMessage: async (sessionId: string, body: Record<string, unknown>) => {
      const req = { method: "POST", url: `/api/v1/agent/sessions/${sessionId}/message`, __body: body } as TestRequest;
      await dispatch(req, `/api/v1/agent/sessions/${sessionId}/message`);
    },
    ...
    postAttachment: async (bytes: Buffer) => {
      ...
      Object.assign(req, { method: "POST", url: "/api/v1/agent/attachments" });
      await dispatch(req, "/api/v1/agent/attachments");
    },
    deleteAttachment: async (id: string) => {
      const req = { method: "DELETE", url: `/api/v1/agent/attachments/${id}` } as http.IncomingMessage;
      await dispatch(req, `/api/v1/agent/attachments/${id}`);
    },
    postSessions: async (body: Record<string, unknown>) => {
      const req = { method: "POST", url: "/api/v1/agent/sessions", __body: body } as TestRequest;
      await dispatch(req, "/api/v1/agent/sessions");
    },
  };
}
