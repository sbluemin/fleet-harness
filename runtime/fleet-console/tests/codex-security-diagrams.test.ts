...
import { encodeMermaidSource } from "../client/src/codex/markdown/renderer";
...
async function loadHydrator(): Promise<InstalledHydrator> {
  ...
  return (await import("../client/src/codex/markdown/diagrams")) as unknown as InstalledHydrator;
}
...
describe("diagram hydrator security", () => {
  ...
  it("source declares strict Mermaid config (static check)", async () => {
    ...
    const src = await fs.readFile("client/src/codex/markdown/diagrams.ts", "utf8");
...
describe("cssColorToHex (pure-JS oklch converter)", () => {
  it("converts percent-formatted oklch to a 7-char #rrggbb hex", async () => {
    const { cssColorToHex } = await import("../client/src/codex/markdown/diagrams");
