...
describe("Console Desktop theme mapping", () => {
  it("keeps native title bar colors in parity with the Console CSS theme tokens", () => {
    ...
    for (const theme of ["maritime", "carbon", "daywatch", "whites", "drydock"] as const) {
...
function readToken(css: string, theme: "base" | "maritime" | "carbon" | "daywatch" | "whites" | "drydock", token: string): [number, number, number] {
