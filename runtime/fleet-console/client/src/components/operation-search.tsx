...
export function OperationSearch({ state }: OperationSearchProps) {
  ...
  const activeOptionId = filteredEntries[clampedSelectedIndex] ? operationOptionId(filteredEntries[clampedSelectedIndex].sessionId) : undefined;
  ...
  useEffect(() => {
    ...
    if (!selected) return;
    resultRefs.current.get(selected.sessionId)?.scrollIntoView({ block: "nearest" });
  }, [clampedSelectedIndex, filteredEntries, state.operationSearchOpen]);

  if (!state.operationSearchOpen) return null;

  const selectEntry = (sessionId: string) => {
    focusOperation(sessionId);
    navigate("/operations");
  ...
  const handleKeyDown = (event: ReactKeyboardEvent<HTMLElement>) => {
    ...
    if (event.key === "Enter") {
      ...
      if (!selected) return;
      event.preventDefault();
      selectEntry(selected.sessionId);
      return;
    }
          ...
          {groups.length > 0 ? groups.map((group) => {
            ...
            <section className="operation-search-section" key={group.theaterId ?? UNASSIGNED_GROUP_KEY} role="group" aria-labelledby={headingId}>
              ...
              {group.entries.map((entry) => {
                ...
                return (
                  <button
                    id={operationOptionId(entry.sessionId)}
                    key={entry.sessionId}
                    ref={(node) => {
                      if (node) resultRefs.current.set(entry.sessionId, node);
                      else resultRefs.current.delete(entry.sessionId);
                    }}
                    type="button"
...
function operationMeta(entry: { readonly cliLabel?: string; readonly status: string }): string {
  return [entry.cliLabel, entry.status].filter(Boolean).join(" · ");
}
...
function operationOptionId(sessionId: string): string {
  return `operation-search-option-${domIdPart(sessionId)}`;
}
