...
`@fleet-console/protocol/desktop` is the source-only shared contract between Fleet Console and Fleet Desktop: protocol version, owner metadata, control env names, resource-root marker format, and canonical Console path resolution.
...
- Import nothing from Console core, Desktop, plugins, or `@dotobokuri/*`; both hosts bundle this module and consume its declared exports only.
