...
import type { ClaudeExecutionEvent, ClaudeGatewayMessage, ClaudeGatewaySdk, ClaudeGatewayTurn } from "@fleet-console/agent-runtime/claude";
...
it("binds the SDK to the host gateway and only the selected model", async () => {
  ...
  await analyst.start();
  expect(sdk.createArgs).toEqual({ baseUrl: BASE_URL, models: ["test-model"], modelPolicy: claudeGatewayModelPolicy });
