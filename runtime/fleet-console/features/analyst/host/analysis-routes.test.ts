...
vi.mock("@fleet-console/analyst", () => ({ AnalystSession: class {} }));

import { analysisArtifactUrl } from "../client/analysis-api.js";
...
describe("Session Analyst server contract", () => {
  ...
  it("reports analysis as ready when the captured transcript exists", async () => {
    ...
    await router.call("GET", "/api/v1/analysis/op/ready");
  ...
  it("wraps hostile artifact CSS with a validated host-owned canvas for every Console theme", async () => {
    ...
    await router.call("POST", "/api/v1/analysis/op/start", START_SELECTION);
    ...
    for (const theme of ["instrument", "maritime", "carbon", "whites"] as const) {
      const path = `/api/v1/analysis/artifacts/hostile?theme=${theme}&canvas=${encodeURIComponent("#123456")}&foreground=${encodeURIComponent("#f0f0f0")}&surface=${encodeURIComponent("#18202b")}&hairline=${encodeURIComponent("#35404d")}&accent=${encodeURIComponent("#65d1ff")}&muted=${encodeURIComponent("#96a0ad")}`;
  ...
  it("falls back instead of embedding hostile, invalid, or overlong theme canvas inputs", async () => {
    ...
    const path = `/api/v1/analysis/artifacts/safe?${query.toString()}`;
  ...
  it("host-gates artifact documents and returns 404 for unknown ids", async () => {
    ...
    await router.call("GET", "/api/v1/analysis/artifacts/missing");
  ...
  it("rejects a malicious Origin through the shared gate for every analysis action", async () => {
    ...
    const requests = [
      ...
      ["GET", "/api/v1/analysis/op/ready"],
      ["POST", "/api/v1/analysis/op/start"],
      ["POST", "/api/v1/analysis/op/message"],
      ["GET", "/api/v1/analysis/op/stream"],
      ["POST", "/api/v1/analysis/op/stop"],
      ["GET", "/api/v1/analysis/artifacts/artifact"],
      ["DELETE", "/api/v1/analysis/op/artifacts"],
    ] as const;
...
function createRouterHarness(initialHostAllowance: boolean) {
  ...
  const operation = { id: "op", pluginId: null, type: "agent", theaterId: "theater", payload: {} as Record<string, unknown>, ts: { createdAt: 0, updatedAt: 0 } };
  const state = {
