...
export function registerAnalysisRoutes(ctx: AnalystHostContext, deps: AnalysisRouteDeps = {}): AnalysisConsoleService {
  ...
  ctx.registerRouter("analysis", async ({ req, res, pathname }) => {
    ...
    { method: "GET", path: "/catalog", summary: "Read Analyst model catalog.", category: "Console Execution", gate: "origin-write", transport: "http" },
    { method: "GET", path: "/stream", summary: "Stream all Analyst events.", category: "Console Execution", gate: "origin-write", transport: "sse" },
    { method: "GET", path: "/artifacts/:artifactId", summary: "Read an Analyst artifact.", category: "Console Execution", gate: "origin-write", transport: "http" },
    { method: "DELETE", path: "/:operationId/artifacts", summary: "Clear Analyst artifacts.", category: "Console Execution", gate: "origin-write", transport: "http" },
    { method: "GET", path: "/:operationId/ready", summary: "Read Analyst readiness.", category: "Console Execution", gate: "origin-write", transport: "http" },
    { method: "POST", path: "/:operationId/start", summary: "Start an Analyst session.", category: "Console Execution", gate: "origin-write", transport: "http" },
    { method: "POST", path: "/:operationId/message", summary: "Send an Analyst message.", category: "Console Execution", gate: "origin-write", transport: "http" },
    { method: "GET", path: "/:operationId/stream", summary: "Stream Analyst session events.", category: "Console Execution", gate: "origin-write", transport: "sse" },
    { method: "POST", path: "/:operationId/stop", summary: "Stop an Analyst session.", category: "Console Execution", gate: "origin-write", transport: "http" },
  ]);

  const unsubscribeDelete = ctx.host.events.subscribe(OPERATION_DELETED_EVENT_CHANNEL, (payload) => {
    if (isOperationDeletedEvent(payload) && payload.pluginId === null) {
...
async function handleCatalog(ctx: AnalystHostContext, req: http.IncomingMessage, res: http.ServerResponse, catalog: () => Promise<AnalysisCatalog>): Promise<boolean> {
  if (req.method !== "GET") return methodNotAllowed(ctx, res);
  ctx.host.http.writeJson(res, 200, await catalog());
  return true;
}

function handleArtifact(ctx: AnalystHostContext, req: http.IncomingMessage, res: http.ServerResponse, artifactId: string, registry: AnalysisRegistry): boolean {
  if (req.method !== "GET") return methodNotAllowed(ctx, res);
  const html = registry.artifactHtml(artifactId);
  if (html === null) {
...
const ARTIFACT_BASE_RULES = [
  ...
  `.fleet-callout{position:relative;border:1px solid var(--fleet-hairline);border-radius:10px;background:var(--fleet-card);padding:11px 13px 11px 28px;margin:0 0 .7em}`,
  `.fleet-callout::before{content:"";position:absolute;left:13px;top:11px;bottom:11px;width:3px;border-radius:2px;background:var(--fleet-hairline-strong)}`,
...
function handleClearArtifacts(ctx: AnalystHostContext, req: http.IncomingMessage, res: http.ServerResponse, operationId: string, registry: AnalysisRegistry): boolean {
...
async function handleReady(ctx: AnalystHostContext, req: http.IncomingMessage, res: http.ServerResponse, operation: OperationNode): Promise<boolean> {
  if (req.method !== "GET") return methodNotAllowed(ctx, res);
  try {
...
async function handleMessage(ctx: AnalystHostContext, req: http.IncomingMessage, res: http.ServerResponse, operationId: string, registry: AnalysisRegistry): Promise<boolean> {
  if (req.method !== "POST") return methodNotAllowed(ctx, res);
  if (!isJsonRequest(req)) return unsupportedMediaType(ctx, res);
  const body = await ctx.host.http.readJsonBody(req);
...
function handleGlobalStream(ctx: AnalystHostContext, req: http.IncomingMessage, res: http.ServerResponse, registry: AnalysisRegistry): boolean {
  if (req.method !== "GET") return methodNotAllowed(ctx, res);
  let closed = false;
...
function handleStream(ctx: AnalystHostContext, req: http.IncomingMessage, res: http.ServerResponse, operationId: string, registry: AnalysisRegistry): boolean {
  if (req.method !== "GET") return methodNotAllowed(ctx, res);
  let closed = false;
  const write = (data: string) => { if (!closed && !res.writableEnded && !res.destroyed) res.write(data); };
  res.writeHead(200, securityHeaders({ "Content-Type": "text/event-stream", "Cache-Control": "no-store", Connection: "keep-alive" }));
...
async function handleStop(ctx: AnalystHostContext, req: http.IncomingMessage, res: http.ServerResponse, operationId: string, registry: AnalysisRegistry): Promise<boolean> {
  if (req.method !== "POST") return methodNotAllowed(ctx, res);
  if (!isJsonRequest(req)) return unsupportedMediaType(ctx, res);
  const body = await ctx.host.http.readJsonBody(req);
...
function methodNotAllowed(ctx: AnalystHostContext, res: http.ServerResponse): true { ctx.host.http.writeJson(res, 405, analysisError(ANALYSIS_ERROR_CODES.catalogInvalid, "Method not allowed.")); return true; }
function unsupportedMediaType(ctx: AnalystHostContext, res: http.ServerResponse): true { ctx.host.http.writeJson(res, 415, analysisError(ANALYSIS_ERROR_CODES.catalogInvalid, "Content-Type must be application/json.")); return true; }
function writeError(ctx: AnalystHostContext, res: http.ServerResponse, status: number, code: keyof typeof ANALYSIS_ERROR_CODES extends never ? never : (typeof ANALYSIS_ERROR_CODES)[keyof typeof ANALYSIS_ERROR_CODES], message: string): void { ctx.host.http.writeJson(res, status, analysisError(code, message)); }
...
function isOperationDeletedEvent(value: unknown): value is { readonly operationId: string; readonly pluginId: string | null } { return !!value && typeof value === "object" && typeof (value as { operationId?: unknown }).operationId === "string" && (value as { pluginId?: unknown }).pluginId === null; }
