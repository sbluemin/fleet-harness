...
vi.mock("../../execution/client/terminal/shared/index.js", () => ({
...
import { agentOperationKind, agentExecution } from "../../execution/client/agent/index.js";
import { applySessionUpdate, removeSession } from "../../execution/client/agent/store.js";
...
describe("Session Analyst entry chip readiness", () => {
  ...
  it("disposing through Operation close POSTs stop and replaces the store", async () => {
    const fetch = vi.fn(async (_pluginId: string | null, path: string) => new Response(
    ...
    const closeOperation = agentExecution.closeOperation;
    ...
    expect(terminateFetch).toHaveBeenCalledWith(`/api/v1/agent/sessions/${OPERATION_ID}`, expect.objectContaining({ method: "DELETE" }));
    expect(fetch).toHaveBeenCalledWith(null, `analysis/${OPERATION_ID}/stop`, expect.objectContaining({ method: "POST", body: "{}" }));
...
function analysisFetch(...readiness: boolean[]): ReturnType<typeof vi.fn> {
  ...
  return vi.fn(async (_pluginId: string | null, path: string) => {
