...
import { fetchRemoteAccessStatus } from "../../settings/client/global-settings-api.js";
import { renderMessage, useT } from "../../../core/client/src/i18n/index.js";
import type { RemoteAccessLink } from "./contracts.js";
  ...
  useEffect(() => {
    ...
    const poll = () => {
      void fetchRemoteAccessStatus(controller.signal)
        .then((status) => {
          if (stopped) return;
          if (status.links.some((entry) => entry.id === link.id)) {
            seen = new Set(status.devices.map((device) => device.id));
            return;
          }
