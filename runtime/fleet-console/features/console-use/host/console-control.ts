...
export const actionSchema = actionObjectSchema.superRefine((value, ctx) => {
  ...
  if (value.kind !== "launch" && (value.model || value.effort || value.viewMode || value.groupId || value.title || value.sessionName || value.disableSubagents || value.dormant !== undefined || value.parentOperationId !== undefined)) ctx.addIssue({ code: "custom", message: "invalid_launch_option" });
