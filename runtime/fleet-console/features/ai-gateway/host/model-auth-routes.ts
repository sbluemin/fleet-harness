...
interface ModelAuthHostContext {
  readonly host: {
...
import { validateKimiAuthKey, validateOpencodeGoAuthKey } from "@fleet-console/ai-gateway";
...
export function registerTerminalModelAuthRoutes(
  ctx: ModelAuthHostContext,
  deps: Pick<TerminalModelAuthRouteDeps, "authService">,
): void {
  ctx.registerRouter("model-auth", createTerminalModelAuthRouter(ctx, {
    ...deps,
    validateApiKey: (provider, apiKey) => MODEL_AUTH_VALIDATORS[provider](apiKey),
  }), [
    { method: "GET", path: "/state", summary: "Read Terminal model provider authentication state.", category: "Console Execution", gate: "loopback", transport: "http" },
    { method: "PUT", path: "/providers/:providerId", summary: "Save Terminal model provider credentials.", category: "Console Execution", gate: "origin-write", transport: "http" },
    { method: "DELETE", path: "/providers/:providerId", summary: "Remove Terminal model provider credentials.", category: "Console Execution", gate: "origin-write", transport: "http" },
  ]);
}
