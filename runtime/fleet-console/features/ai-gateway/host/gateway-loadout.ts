import { parseGatewayQuotaSnapshot, type GatewayQuotaSnapshot } from "@fleet-console/ai-gateway";

// Gateway가 소유한 단일 쿼터 캐시를 HTTP 소비자도 동일하게 읽는다.
const QUOTA_SUMMARY_PATH = "/api/v1/ai-gateway/quota";
...
/**
 * Read the Gateway-owned quota cache as the loadout's allowance source.
 *
...
export async function readConsoleQuotaSnapshot(
  ...
  try {
    ...
    return parseGatewayQuotaSnapshot(JSON.parse(text));
  } catch {
    return undefined;
  } finally {
    clearTimeout(timer);
