...
async function fetchModelAuthState(signal?: AbortSignal): Promise<ModelAuthState> {
  const response = await fetch("/api/v1/model-auth/state", { signal });
...
async function signInModelProvider(provider: string, apiKey: string, signal?: AbortSignal): Promise<ModelAuthMutationResult> {
  const response = await fetch(`/api/v1/model-auth/providers/${encodeURIComponent(provider)}`, {
    method: "PUT",
...
import { fetchAgentState } from "../../execution/client/agent/api.js";
import { hydrateAgentClis } from "../../execution/client/agent/store.js";
