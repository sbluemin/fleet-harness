...
- `models.json`, `benchmarks.json`, and `src/models.ts` are the central catalog sources. Keep current generations only. `capabilityClass` is the benchmark score band where evidence exists and the vendor's lineup claim only where it does not; join benchmarks only by explicit `benchmarkKey`.
- Before refreshing benchmark evidence, read [benchmark-methodology.md](benchmark-methodology.md). Benchmark evidence comes from one public source, LLM Stats; re-check its reuse terms and keep the user-visible attribution. Retain the retrieval date, artifact hash, and source model identity; record rejected sources and excluded models.
- Every admitted model carries the full fixed metric set. Never impute a missing metric, borrow another generation or an ambiguous revision, or mix in figures from another source. Routing aliases such as Cursor Auto carry no fixed-model benchmark evidence.
- Benchmark figures are the source's published model-level indexes, not absolute accuracy, provider billing facts, or per-effort measurements. Keep models without evidence available in the provider catalog without a benchmark.
