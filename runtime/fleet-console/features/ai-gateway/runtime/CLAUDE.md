...
- `models.json`, `benchmarks.json`, and `src/models.ts` are the central catalog sources. Keep current generations only. Claude entries are Claude Code aliases whose version, wire id, and effort ladder come from the installed CLI's `supportedModels()`; never pin a Claude version in `models.json`. `capabilityClass` is a vendor claim, not a measurement; join benchmarks only by explicit `benchmarkKey`.
...
- Normalize only the complete intersection: every admitted model has the identical required sources and metrics, with the same source weights and source-specific evaluation conditions. Never impute missing scores, borrow another generation or effort, mix fallback systems with a single model, or silently shrink the required source set. Routing aliases carry no fixed-model benchmark evidence.
...
- Never suppress declared default-valued arguments: intentional input is indistinguishable from pollution. OpenCode Chat Completions guarantees undeclared-key pruning on completed arguments, not a strict-mode outbound rewrite. Unreadable `pattern` values are dropped outbound so a backend that cannot compile them does not fail the request.
