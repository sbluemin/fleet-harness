...
import { createClaudeExecutionLoop, createClaudeGatewaySdk } from "@fleet-console/agent-runtime/claude";

import { AnalystSession, type AnalystEvent } from "@fleet-console/analyst";
import { PROMPT_REFINE_MAX_CHARS, type OperationNode, type PromptRefinement, type PromptRefinePurpose } from "@fleet-console/sdk/plugin";
...
async function defaultRunOneShot(input: { readonly baseUrl: string; readonly model: string; readonly system: string; readonly user: string }): Promise<string> {
  ...
  const loop = createClaudeExecutionLoop({
    createSdk: () => createClaudeGatewaySdk({ modelPolicy: claudeGatewayModelPolicy, baseUrl: input.baseUrl, models: [input.model] }),
    buildTurn: () => ({
      model: input.model,
      effort: "medium",
...
export function registerExperimentRoutes(ctx: ConsoleRuntimeContext, deps: ExperimentRouteDeps): SessionWatchService {
