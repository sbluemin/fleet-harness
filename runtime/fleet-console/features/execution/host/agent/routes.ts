...
export async function registerAgentRoutes(
  ...
  registerRouter(ctx, "agent", api.handle, [
    ...
    { method: "DELETE", path: "/sessions/:sessionId", summary: "Delete an Agent session.", category: "Console Execution", gate: "origin-write", transport: "http" },
    { method: "POST", path: "/sessions/:sessionId/resume", summary: "Resume an Agent session.", category: "Console Execution", gate: "origin-write", transport: "http" },
    { method: "POST", path: "/sessions/:sessionId/message", summary: "Deliver a prompt to an Agent session.", category: "Console Execution", gate: "origin-write", transport: "http" },
    { method: "POST", path: "/sessions/:sessionId/chat", summary: "Switch an Agent session to chat mode.", category: "Console Execution", gate: "origin-write", transport: "http" },
    { method: "DELETE", path: "/sessions/:sessionId/chat", summary: "Leave chat mode for an Agent session.", category: "Console Execution", gate: "origin-write", transport: "http" },
...
async function createAgentApi(ctx: ConsoleRuntimeContext, terminalRuntime: TerminalRuntime, deps: AgentRouteDeps) {
  ...
  const unsubscribeRename = ctx.host.events.subscribe(OPERATION_RENAMED_EVENT_CHANNEL, (payload) => {
    ...
    if (payload.pluginId !== null || payload.type !== AGENT_OPERATION_TYPE) return;
  ...
  async function resolveChatSeed(node: OperationNode): Promise<ChatSeedResolution> {
    ...
    return {
      ...
      seed: {
        ...
        modHookToken: deps.aiGateway?.modHookToken,
        model,
        ...
        /**
         * 보낸 쪽이 관측한 세션 간 메시지를 받는 쪽 원장에 옮긴다. 대상은 **이름이 아니라 등록**으로
         * 찾는다: 런치가 CLI에 준 `--name`과 Operation payload에 적힌 `sessionName`이 같은 출처이므로,
         * 정확히 하나가 걸릴 때만 그 Operation이 그 이름의 주인이다. 0개(이 Console 밖 세션·서브에이전트·
         * `main`)나 2개 이상이면 아무 줄도 세우지 않는다 — 짐작으로 세운 줄은 남의 말을 남의 원장에 놓는다.
         *
  ...
  async function handleExit(operationId: string): Promise<void> {
    ...
    if (providerSession) {
      ...
      // 단 Chat이 인수한 세션의 PTY 종료는 표면 전환이지 휴면이 아니다 — 자식은 SDK 위에서 계속
      // 돌고 cwd는 턴 경계마다 세션이 알리므로, 추적과 투영을 그대로 둔다.
      if (observability.getTerminalSessionInfo(operationId)?.chatActive !== true) {
        workspaceContext.forget(operationId);
        observability.setTerminalSessionWorkspace(operationId, null);
      }
...
function isOperationDeletedEventPayload(value: unknown): value is { readonly operationId: string; readonly pluginId: string | null } {
...
function isOperationRestoredEvent(value: unknown): value is { readonly operationId: string; readonly pluginId: string | null; readonly type: string } {
  if (!value || typeof value !== "object") return false;
  const event = value as { readonly operationId?: unknown; readonly pluginId?: unknown; readonly type?: unknown };
  return typeof event.operationId === "string" && event.pluginId === null && typeof event.type === "string";
}
...
function readOptionalAgentCliId(value: unknown, res: Parameters<ConsoleRuntimeContext["host"]["http"]["writeJson"]>[0]): AgentCliId | undefined | false {
  if (value === undefined) return undefined;
