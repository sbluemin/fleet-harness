 ...
 *
 * 이 스킴은 브라우저가 따라갈 수 없다. 링크를 여는 주체는 Fleet Desktop 하나뿐이며, 브라우저
 * 원격 접속은 별도 결정 전까지 열지 않는다.
 *
...
export function encodeAccessLink(payload: Omit<AccessLinkPayload, "v">): string {
  ...
  return `${ACCESS_LINK_PREFIX}${toBase64Url(JSON.stringify(envelope))}`;
}
...
export function parseAccessLink(input: string): ValidatedAccessLink {
  ...
  if (/[\u0000-\u001f\u007f\s]/u.test(candidate)) throw invalidLink();
  let url: URL;
  try {
  ...
  if (url.hostname !== ACCESS_LINK_HOST || url.port !== "") throw invalidLink();
...
function decodePayload(code: string): unknown {
  ...
  let text: string;
  try {
    text = Buffer.from(code, "base64url").toString("utf8");
  } catch {
    throw invalidLink();
  }
  try {
