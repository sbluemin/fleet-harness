...
export const macOSComputerUsePlatform: ComputerUsePlatform = {
  toolDescriptions: {
    ...
    computer_state: "Read a Mac app's key-window accessibility tree and optional model-facing screenshot. Use an unambiguous bundle ID, exact name, or absolute .app path. Returns a fresh snapshotId. Fleet keeps one valid snapshot across apps; any observation invalidates the previous one. Dispatched actions consume it even on native error; not_started validation errors preserve it. Only use current element indices. actionSchemas are sent once per version per broker session; request includeActionSchemas:true after compaction or if missing. Content may be sent to the selected model provider. Settings opt-in pre-approves device permissions; no per-app confirmation. " + OBSERVATION_GUIDANCE,
    computer_action: "Perform ONE authorized GUI action using the latest app snapshotId. completed means the native call returned, NOT verified effect; inspect the returned observation. Success returns a fresh snapshotId: use it directly without another computer_state. observation controls the post-action output, not permission to use coordinates from a text-only input snapshot. Never auto-retry timeouts or repeat actions when observation fails; read computer_state only. Follow user scope and harness safety policy for consequential actions; describe the effect in reason. Settings opt-in needs no further device-permission confirmation. type_text accepts printable ASCII only. For Hangul/Unicode/multiline use set_value with the complete field value (replacement, not insertion); verify before explicit press_key submission. Empty set_value is unsupported; clearing requires explicit operations on a verified editable field, never automatic select-all/delete fallback. Actions may foreground apps; opening chats can mark them read. Copy/cut overwrites the clipboard; no clipboard/focus restoration. Cannot target arbitrary MCP servers or shell commands. " + OBSERVATION_GUIDANCE,
  },
