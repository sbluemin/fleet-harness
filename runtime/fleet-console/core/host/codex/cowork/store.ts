...
export class CoworkStore {
  ...
  async create(workspaceId: string, entryId: string, body: string, baseVersion = 0, baseHash = hash(body)): Promise<CoworkSessionRecord> {
    ...
    const record: CoworkSessionRecord = { id, workspaceId, entryId, state: "idle", revision: 0, draft: body, baseHash, baseVersion, selection: null, annotations: [], createdAt: now, updatedAt: now };
  ...
  async appendTranscript(workspaceId: string, sessionId: string, turn: CoworkTranscriptTurn) { const all = await this.transcript(workspaceId, sessionId); all.push(turn); await this.atomic(join(this.dir(workspaceId, sessionId), "transcript.json"), all); }
  ...
  async appendEvent(workspaceId: string, sessionId: string, event: CoworkStoredEvent) { const all = await this.events(workspaceId, sessionId); all.push(event); await this.atomic(join(this.dir(workspaceId, sessionId), "events.json"), all.slice(-100)); }
