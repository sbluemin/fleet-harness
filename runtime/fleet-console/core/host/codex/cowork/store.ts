...
export class CoworkStore {
  ...
  // read→검사→write 전 구간을 세션 단위로 직렬화해야 동시 draft CAS가 유실 없이 거부된다.
  async update(workspaceId: string, sessionId: string, fn: (value: CoworkSessionRecord) => CoworkSessionRecord): Promise<CoworkSessionRecord> { return this.serialize(`${workspaceId}:${sessionId}:session`, async () => { const old = await this.get(workspaceId, sessionId); if (!old) throw new Error("cowork_session_not_found"); const next = fn(old); await this.save(next); return next; }); }
  async activeForEntry(workspaceId: string, entryId: string): Promise<CoworkSessionRecord | null> { const id = this.writers.get(`${workspaceId}:${entryId}`); if (!id) return null; const s = await this.get(workspaceId, id); return s && s.state !== "closed" && s.state !== "applied" ? s : null; }
