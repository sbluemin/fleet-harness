...
const services = new Map<string, CoworkService>();
export async function handleCoworkRequest(request: IncomingMessage, response: ServerResponse, context: { workspaceId: string; paths: MemoryPaths; dataDir?: string; allowedOrigins: Set<string>; port: number }): Promise<boolean> {
  const url = new URL(request.url ?? "/", "http://127.0.0.1");
  if (!url.pathname.startsWith("/api/cowork")) return false;
  const service = services.get(context.workspaceId) ?? new CoworkService(new CoworkStore(context.dataDir ?? context.paths.root), context.paths); services.set(context.workspaceId, service);
  const parts = url.pathname.split("/").filter(Boolean);
  if (request.method === "GET" && parts.length === 3 && parts[2] === "options") return json(response, 200, { clis: ["codex", "claude", "cursor", "opencode"], models: [], efforts: [] });
