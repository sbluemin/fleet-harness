...
export async function fetchAgentState(signal?: AbortSignal): Promise<readonly AgentCliMetadata[]> {
  const response = await fetch("/api/v1/agent/state", { signal });
...
export async function fetchClaudeBuiltInAgents(signal?: AbortSignal, options?: { readonly refresh?: boolean }): Promise<ClaudeBuiltInAgentsState> {
  ...
  const response = options?.refresh
    ? await fetch("/api/v1/agent/agent-cli/claude-agents/refresh", { method: "POST", signal })
    : await fetch("/api/v1/agent/agent-cli/claude-agents", { signal });
  await assertOk(response);
...
export async function fetchSessions(signal?: AbortSignal): Promise<readonly SessionInfo[]> {
  const response = await fetch("/api/v1/agent/sessions", { signal });
  await assertOk(response);
...
export async function messageAgentSession(sessionId: string, text: string, attachmentIds?: readonly string[], signal?: AbortSignal): Promise<void> {
  const response = await fetch(`/api/v1/agent/sessions/${encodeURIComponent(sessionId)}/message`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function answerAgentChatAsk(
  ...
  const response = await fetch(`/api/v1/agent/sessions/${encodeURIComponent(sessionId)}/chat-answer`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function stopAgentChatTurn(sessionId: string, signal?: AbortSignal): Promise<void> {
  const response = await fetch(`/api/v1/agent/sessions/${encodeURIComponent(sessionId)}/chat-stop`, { method: "POST", signal });
  if (!response.ok) {
    const payload = await response.json().catch(() => null) as { readonly error?: unknown } | null;
    throw new AgentApiError(response.status, typeof payload?.error === "string" ? payload.error : `Agent plugin request failed: ${response.status}`);
  }
...
export async function readAgentChatJobDetail(
  ...
  const response = await fetch(`/api/v1/agent/sessions/${encodeURIComponent(sessionId)}/chat-job${query}`, { signal });
...
export async function readAgentChatCatalog(
  ...
  const response = await fetch(`/api/v1/agent/sessions/${encodeURIComponent(sessionId)}/chat-catalog`, { signal });
  if (response.status === 404 || response.status === 409) return null;
  if (!response.ok) {
    const payload = await response.json().catch(() => null) as { readonly error?: unknown } | null;
    throw new AgentApiError(response.status, typeof payload?.error === "string" ? payload.error : `Agent plugin request failed: ${response.status}`);
...
export async function exitAgentChat(sessionId: string, signal?: AbortSignal): Promise<void> {
  const response = await fetch(`/api/v1/agent/sessions/${encodeURIComponent(sessionId)}/chat`, { method: "DELETE", signal });
  if (!response.ok) {
    const payload = await response.json().catch(() => null) as { readonly error?: unknown } | null;
    throw new AgentApiError(response.status, typeof payload?.error === "string" ? payload.error : `Agent plugin request failed: ${response.status}`);
  }
...
export async function uploadLaunchAttachment(file: Blob, signal?: AbortSignal): Promise<{ readonly id: string }> {
  const response = await fetch("/api/v1/agent/attachments", {
    method: "POST",
...
export async function terminateAgentSession(sessionId: string, signal?: AbortSignal): Promise<void> {
  const response = await fetch(`/api/v1/agent/sessions/${encodeURIComponent(sessionId)}`, { method: "DELETE", signal });
  await assertOk(response);
}
