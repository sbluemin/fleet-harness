 ...
 */
export function OperationWorkspaceContext({ workspace, className, titled = true }: {
  readonly workspace: OperationWorkspace;
  readonly className?: string;
  /**
   * 잘린 값의 전체를 네이티브 툴팁으로 여는지. 사이드바 칩은 겨누면 상세 카드가 같은 값을 온전히
   * 들고 뜨므로 끈다 — 켜 두면 카드와 OS 툴팁이 함께 떠서 서로를 가린다. 팔레트는 카드가 없어 켠다.
   */
  ...
  return (
    <span className={classes} aria-hidden="true">
      {workspace.branch ? (
        <span className="operation-context-branch" title={titled ? t("sidebar.chip.branchTitle", { branch: workspace.branch }) : undefined}>{workspace.branch}</span>
      ) : null}
      ...
      {folderLabel ? (
        <span
          ...
          title={!titled ? undefined : workspace.outside ? t("sidebar.chip.outsideTitle", { folder: folderLabel }) : t("sidebar.chip.folderTitle", { folder: folder ?? "" })}
        >
          {folderLabel}
        </span>
      ) : null}
