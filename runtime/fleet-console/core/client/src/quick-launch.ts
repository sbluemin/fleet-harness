...
// `ultracode` 인식 문법은 컴포저 블록이 한 벌로 소유한다 — Quick Launch와 채팅 컴포저가 같은
// 규칙을 조립하므로, 정규식·해제 만료·해제 caret 판정은 SDK 부품에서 단일 출처로 온다. 여기서는
// 단위 테스트와 컴포넌트가 기존 경로로 계속 읽도록 그대로 다시 내보낸다.
export {
  isUltracodeDisarmCaret,
  nextUltracodeIgnored,
  readUltracodeTokens,
} from "@fleet-console/sdk/composer";
export type { UltracodeToken } from "@fleet-console/sdk/composer";
