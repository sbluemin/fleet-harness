...
export function useCodexReaderUrlSync(): void {
  ...
  useEffect(() => {
    ...
    const signature = `${entryId ?? ""}|${wantExpanded ? VIEW_FULL : ""}`;
  ...
  }, [theaterId, reader, expanded, location.search]);
  ...
  useEffect(() => {
    ...
    if (currentEntry === nextEntry && currentView === nextView) return;
    ...
    writtenRef.current = `${nextEntry ?? ""}|${nextView ?? ""}`;
...
function readTarget(search: string): ReaderTarget | null {
  ...
  return { entryId, expanded: params.get(VIEW_PARAM) === VIEW_FULL };
}
