...
export const FEATURE_TOURS: readonly FeatureTour[] = [
  ...
  },
  {
    ...
    // 짚는다. War Room·companion·Fleet Map에서는 캡션 드래그가 잠겨 스냅이 없고, 전체 칸을
    // 쥔 화면에서는 나눠 쓸 칸이 없으니 캔버스 상태 클래스로 Cruise만 남긴다. 모두 정렬 묶음도
    // 분할이 막혀 있어 뺀다. 덱 카드·최소화 패널의 캡션은 보이지 않으므로 뺀다.
    // 두 번째 스텝은 캡션의 전체 칸 버튼 — 머무르면 같은 프리셋 메뉴가 열린다(data-snap-tour).
    // 첫 방문의 모드 투어와 겹치지 않게 한 박자 미룬다.
    spotlight: null,
    deferAfterAnotherTour: true,
    walkthrough: [
      {
        anchor: ".operations-canvas:not(.is-triage):not(.is-panel-snap-full):not(.is-companion-layout):not(.is-fleet-map) .canvas-operation:not(.is-align-held):not(.is-deck-tile):not(.is-minimized) .canvas-operation-titlebar",
        titleKey: "featureTour.cruiseSnap.step1Title",
        bodyKey: "featureTour.cruiseSnap.step1Body",
      },
      {
        anchor: '.operations-canvas:not(.is-triage):not(.is-panel-snap-full):not(.is-companion-layout):not(.is-fleet-map) .canvas-operation:not(.is-align-held):not(.is-deck-tile):not(.is-minimized) [data-snap-tour="menu"]',
