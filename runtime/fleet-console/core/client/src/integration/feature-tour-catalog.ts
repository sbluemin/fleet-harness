...
export const FEATURE_TOURS: readonly FeatureTour[] = [
  ...
  },
  {
    ...
    // 짚는다. War Room·최대화·companion·Fleet Map에서는 캡션 드래그가 잠겨 스냅이 없으니
    // 캔버스 상태 클래스로 Cruise만 남긴다. 모두 정렬 묶음도 분할이 막혀 있어 뺀다.
    // 덱 카드·최소화 패널의 캡션은 보이지 않으므로 뺀다.
    ...
    walkthrough: [
      {
        anchor: ".operations-canvas:not(.is-triage):not(.is-panel-maximized):not(.is-companion-layout):not(.is-fleet-map) .canvas-operation:not(.is-align-held):not(.is-deck-tile):not(.is-minimized) .canvas-operation-titlebar",
        titleKey: "featureTour.cruiseSnap.step1Title",
        bodyKey: "featureTour.cruiseSnap.step1Body",
      },
      {
        anchor: '.operations-canvas:not(.is-triage):not(.is-panel-maximized):not(.is-companion-layout):not(.is-fleet-map) .canvas-operation:not(.is-align-held):not(.is-deck-tile):not(.is-minimized) [data-snap-tour="menu"]',
