...
const manifest = { version: "22.23.1", source: "https://node.invalid", targets: { "linux-arm64": { archive: "node-v22.23.1-linux-arm64.tar.xz", sha256: "0294e8b915ab75f92c7513d2fcb830ae06e10684e6c603e99a87dbf8835389c1" }, "linux-x64": { archive: "node-v22.23.1-linux-x64.tar.xz", sha256: "x64" } } };

describe("remote Node runtime", () => {
  it("maps only Linux uname pairs to the pinned arm64 archive", async () => {
    const ssh = { run: vi.fn(async () => ({ stdout: "Linux\x0aaarch64\x0a", stderr: "", exitCode: 0 })) };
    await expect(detectRemotePlatform(parseSshTarget("devbox"), manifest, ssh)).resolves.toMatchObject({ targetKey: "linux-arm64", archive: manifest.targets["linux-arm64"] });
