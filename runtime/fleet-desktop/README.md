...
- **J5 attach:** identity-verify and display a healthy canonical Console (CLI- or Desktop-owned) without claiming its lifecycle; bootstrap and update work are skipped.
...
First-install failure **Retry**/**Quit** and J6 update prompts remain native. **Connect to Runtime…** opens a Desktop-owned local child modal with no JavaScript, accepts only literal `127.0.0.1:port`, verifies the read-only pairing identity, and transactionally loads that Console. Closing the window follows normal macOS behavior and hides to tray on Windows/Linux; a second launch restores the existing window.
...
- **CLI-owned daemon exists:** Desktop identity-verifies and displays it without deleting its lock, signaling it, or taking lifecycle ownership. Desktop Quit leaves it running.
- **Healthy foreign Console cannot prove pairing identity:** Desktop shows a native error and exits without signaling or claiming that Console.
