...
const SCRIPTS: Readonly<Record<RemoteOperation, string>> = {
  ...
  promote_runtime_path: "set -eu; if [ ! -e \"$HOME/$2\" ] && [ -e \"$HOME/$2.old\" ]; then mv \"$HOME/$2.old\" \"$HOME/$2\"; fi; rm -rf \"$HOME/$2.old\"; if [ -e \"$HOME/$2\" ]; then mv \"$HOME/$2\" \"$HOME/$2.old\"; fi; mv \"$HOME/$1\" \"$HOME/$2\"; rm -rf \"$HOME/$2.old\"",
  ...
  install_console: "set -eu; nodebin=$(dirname \"$HOME/$1\"); prefix=\"$HOME/$3\"; mkdir -p \"$prefix\"; : > \"$prefix/.npmrc\"; : > \"$prefix/.npmrc-global\"; PATH=\"$nodebin:$PATH\"; export PATH; unset NODE_OPTIONS FLEET_CONSOLE_OWNER_ID FLEET_CONSOLE_OWNER_KIND FLEET_CONSOLE_PROTOCOL_VERSION FLEET_CONSOLE_RESOURCE_ROOT FLEET_CONSOLE_DIR FLEET_CONSOLE_DESKTOP_VERSION FLEET_CONSOLE_DESKTOP_DEVELOPMENT; for v in $(env | sed -n 's/^\\(ELECTRON_[A-Za-z0-9_]*\\)=.*/\\1/p'); do unset \"$v\"; done; for v in $(env | grep -iE '^npm_config_' | cut -d= -f1); do unset \"$v\"; done; npm_config_userconfig=\"$prefix/.npmrc\" npm_config_globalconfig=\"$prefix/.npmrc-global\" npm_config_registry=https://registry.npmjs.org/ \"$HOME/$1\" \"$HOME/$2\" install --prefix \"$prefix\" --no-audit --no-fund --loglevel=error \"$4\"; rm -f \"$prefix/.npmrc\" \"$prefix/.npmrc-global\"",
  ...
  start_console: "set -eu; loginsh=\"${SHELL:-/bin/sh}\"; loginpath=$(\"$loginsh\" -ilc 'printf %s \"$PATH\"' 2>/dev/null || printf %s \"\"); loginpath_valid=1; case \"$loginpath\" in \"\"|:*|*:|*[[:cntrl:]]*) loginpath_valid=0 ;; esac; loginpath_remaining=$loginpath; while [ \"$loginpath_valid\" -eq 1 ] && [ -n \"$loginpath_remaining\" ]; do case \"$loginpath_remaining\" in *:*) loginpath_entry=${loginpath_remaining%%:*}; loginpath_remaining=${loginpath_remaining#*:} ;; *) loginpath_entry=$loginpath_remaining; loginpath_remaining= ;; esac; case \"$loginpath_entry\" in /*) ;; *) loginpath_valid=0 ;; esac; done; if [ \"$loginpath_valid\" -eq 1 ]; then PATH=\"$loginpath:$PATH\"; export PATH; fi; unset NODE_OPTIONS FLEET_CONSOLE_OWNER_ID FLEET_CONSOLE_OWNER_KIND FLEET_CONSOLE_PROTOCOL_VERSION FLEET_CONSOLE_RESOURCE_ROOT FLEET_CONSOLE_DIR FLEET_CONSOLE_DESKTOP_VERSION FLEET_CONSOLE_DESKTOP_DEVELOPMENT; for v in $(env | sed -n 's/^\\(ELECTRON_[A-Za-z0-9_]*\\)=.*/\\1/p'); do unset \"$v\"; done; cd \"$HOME/$1\"; FLEET_CONSOLE_OWNER_KIND=desktop FLEET_CONSOLE_OWNER_ID=\"$4\" FLEET_CONSOLE_PROTOCOL_VERSION=\"$5\" FLEET_CONSOLE_DESKTOP_VERSION=\"$6\" FLEET_CONSOLE_DIR=\"$HOME/$7\" FLEET_CONSOLE_RESOURCE_ROOT=\"$HOME/$1\" nohup \"$HOME/$2\" \"$HOME/$3\" serve >/dev/null 2>&1 & printf %s \"$!\"",
};
...
function defaultSpawn(file: string, args: readonly string[], options: SpawnOptionsWithoutStdio): OpenSshProcess {
  ...
  return { pid: child.pid, stdout: child.stdout, stderr: child.stderr, stdin: child.stdin, terminate: () => { child.kill(); }, exited: new Promise((resolve) => child.once("close", (code, signal) => resolve({ code, signal }))) };
}
