...
`runtime/fleet-cli/src/admiral/protocols/` owns the modular prompt policy system that governs how the host agent operates, composed of **Standing Orders** and **Protocols**. Any new injectable composition must use explicit `create*(deps): Interface` pure factories and must not introduce DI containers or frameworks.
