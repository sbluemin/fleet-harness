...
Claude sessions also receive a Fleet-managed `SessionStart` hook that bootstraps through the current Fleet entry by absolute path (`node <entry> hook subagents-context`, or `node --import <tsx-loader> <entry> hook subagents-context` for development TypeScript entries), so the hook does not depend on `fleet` being present on `PATH`; Codex and Cursor do not receive this hook.
