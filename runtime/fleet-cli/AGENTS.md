...
Direct dependencies on execution-engine packages are generally forbidden. Execution and model catalog access flow through `fleet-infra` and the Fleet orchestration packages. The Job Bar functionality is fully absorbed into `fleet-cli`.
...
- The DI layer order is one-way: `fleet-cli` -> `fleet-carriers` -> `fleet-infra`; `fleet-mcp-server` is a generic leaf dependency consumed through public APIs.
