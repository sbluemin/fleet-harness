...
describe("update registry", () => {
  ...
  it("delegates fleet-cli registry lookup to the shared core-agent helper", async () => {
    mockedFetchLatestVersion.mockResolvedValue("1.2.3");

    await expect(fetchLatestFleetCliVersion("latest")).resolves.toBe("1.2.3");

    expect(mockedFetchLatestVersion).toHaveBeenCalledWith("@dotobokuri/fleet-cli", "latest");
  });
});
