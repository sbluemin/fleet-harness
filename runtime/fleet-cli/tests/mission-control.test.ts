...
describe("Mission Control controller", () => {
  ...
  it("includes carrier/wiki/queue counts and stable channel readout", () => {
    const lines = renderMissionControl(80, {
      ...
      lastExit: undefined,
      loadedCounts: { carriers: 6, queuedPatches: 3, wikiEntries: 17 },
    ...
    expect(plainOutput).toContain("6 carriers");
  ...
  it("anchors launcher status counts and update state above the root footer", () => {
    const controller = createTestController({
      loadedCounts: { carriers: 6, queuedPatches: 0, wikiEntries: 17 },
    ...
    const countsIndex = plainLines.findIndex((line) => line.includes("✓ 6 carriers"));

    expect(output).toContain("✓ 6 carriers");
  ...
  it("renders about panel with counts and placeholder docs link", () => {
    ...
    expect(renderPlain(controller)).toContain("Carriers      : 6");
    ...
    expect(output).toContain("Carriers      : \x1b[38;2;254;188;56m6\x1b[0m");
