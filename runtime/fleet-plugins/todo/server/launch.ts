 ...
 * Console 이 조율자에게 넣는 통지는 셋뿐이다: 연결됨 · 쿠킹 요청 · 담당 세션이 떴다(이름 목록).
 * 「콘솔 사용」은 조율자에게만 켠다. 담당은 도구 없이 자기 단계만 한다.
 */

export interface LaunchService {
  describe(): { readonly available: boolean };
  startCoordinator(itemId: string, options?: LaunchOptions): Promise<{ readonly item: TodoItem; readonly operationId: string }>;
  linkCoordinator(itemId: string, operationId: string, options?: LaunchOptions): Promise<{ readonly item: TodoItem }>;
...
export function createLaunchService(ctx: FleetPluginServerContext, store: TodoStore): LaunchService {
  const control = () => {
    const capability = ctx.host.consoleControl;
    if (!capability) throw new TodoStoreError("launch_unavailable");
    return capability;
  };
  const item = (itemId: string) => {
    const found = store.find(itemId);
    if (!found) throw new TodoStoreError("unknown_item");
    return found;
  };
  const step = (current: TodoItem, stepId: string): TodoStep => {
    const found = current.steps.find((candidate) => candidate.id === stepId);
    if (!found) throw new TodoStoreError("unknown_step");
    return found;
  };
  const operation = (operationId: string) => {
    const node = ctx.host.operations.get(operationId);
    if (!node) throw new TodoStoreError("unknown_operation");
    return node;
  };
  const preset = (current: TodoItem): { model?: string; effort?: string; view?: LaunchView } => current.launch;

  /** payload 에 포인터·콘솔 사용 표식을 남긴다. */
  const mark = (operationId: string, todo: { itemId: string; stepId?: string; role: "coordinator" | "step" }, consoleUse: PromptLanguage | null) => {
    const node = ctx.host.operations.get(operationId);
    if (!node) return;
    const payload: Record<string, unknown> = { ...node.payload, todo };
  ...
  const unmark = (operationId: string) => {
    const node = ctx.host.operations.get(operationId);
    if (!node || !node.payload.todo) return;
  ...
  const launch = async (input: { theaterId: string; title: string; text?: string; view?: LaunchView; sessionName: string; model?: string; effort?: string; groupId: string | null; subagents?: boolean }): Promise<string> => {
    ...
    }, `todo:launch:${randomUUID()}`);
    if (!receipt.operationId) throw new TodoStoreError(receipt.error ?? "launch_failed");
  ...
  const asStoreError = (error: unknown): never => {
    ...
    throw new TodoStoreError(/^[a-z_]{1,64}$/.test(code) ? code : "launch_failed");
  };
  ...
  const claim = async <T,>(key: string, run: () => Promise<T>): Promise<T> => {
    if (pending.has(key)) throw new TodoStoreError("slot_taken");
    pending.add(key);
   ...
   * (라우팅이 꺼져 있거나 실패하면 셰프 프리셋으로). model 은 그 모델·강도, 없으면 셰프 프리셋.
   */
  const routeStep = async (current: TodoItem, target: TodoStep, index: number): Promise<{ model?: string; effort?: string; because?: string }> => {
  ...
  const stepLaunchPreset = async (current: TodoItem, target: TodoStep, index: number): Promise<{ model?: string; effort?: string }> => {
  ...
  /**
   * 담당 세션은 셰프가 위임하는 순간 뜬다 — 할 일은 담당에게 프롬프트를 보내지 않는다. 사람이 미리 배정한
   * 모델(route/model)은 그때 쓸 모델 선택으로만 작용한다.
   */
  const delegateStep = async (itemId: string, stepId: string, language: PromptLanguage): Promise<{ item: TodoItem; operationId: string; session: string }> => {
    let current = item(itemId);
    const target = step(current, stepId);
    if (target.done) throw new TodoStoreError("step_done");
    if (target.slot) throw new TodoStoreError("slot_taken");
  ...
  const service: LaunchService = {
    ...
    startCoordinator: (itemId, options) => claim(itemId, async () => {
      ...
      // 시작은 쿠킹을 끝내고 검토 대기를 거둔다 — 여기서부터 계획·단계 추가가 담당을 띄운다.
      ...
      // 단계 하나짜리 할 일은 조율자가 곧 담당이다 — Operation 하나로 끝내고, 브리프도 그렇게 말한다.
      const alone = open.length === 1 && !open[0]!.slot ? open[0] : undefined;
      const brief = alone ? { self: alone } : {};
      // 담당과 대화하는 셰프는 CLI 여야 하므로 위임 단계가 하나라도 있으면 터미널로 연다. 담당 자체는 셰프가 위임할 때 뜬다.
    ...
    requestPlan: (itemId, options) => claim(itemId, async () => {
      ...
      let current = item(itemId);
      if (current.done) throw new TodoStoreError("item_done");
      // 쿠킹은 계획과 메모만이다 — 단계 수행도, 담당 기동도 「시작」이 한다.
    ...
    async stop(itemId) {
      ...
      for (const operationId of ids) {
        ...
        try { await control().request({ kind: "interrupt", operationId }, `todo:stop:${randomUUID()}`); interrupted += 1; }
    ...
    operationDeleted(operationId) {
      // 셰프가 닫히면 그 항목의 담당 Operation 도 닫는다 — 담당은 셰프 없이는 할 일이 없다.
