  ...
  const tool: PluginMcpTool = {
    ...
    execute: async (raw, context) => {
      ...
      try {
        ...
        // 셰프가 마지막으로 읽은 뒤 사람이 바꾼 것이 있으면(edited) 그 계획은 옛 보드로 짠 것이다 — 사람의 편집을 덮지 않게 거절하고 다시 읽힌다.
        if (key === "plan" && item.edited) return refuse("board_changed", { hint: "The person changed this item since you last read it. Read it again (view item or mine), then plan." });
