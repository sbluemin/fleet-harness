...
afterEach(() => {
  ...
  for (const [theaterId, repoRel] of repoScopes) dropRepoViewState(theaterId, repoRel);
  theaterIds.clear();
  repoScopes.clear();
});

describe("repository panel state cache", () => {
  it("round-trips entries in both caches", () => {
  ...
  it("moves a read key to the most recently used position", () => {
    ...
    keys.forEach(([theaterId, repoRel], index) => writeRepoViewState(theaterId, repoRel, viewEntry(String(index))));
    expect(readRepoViewState(keys[0]![0], keys[0]![1])).not.toBeNull();
  ...
  it("keeps the two maps independent for keys with the same suffix", () => {
