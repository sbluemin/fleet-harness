...
type Refs = { branches: { name: string; current: boolean }[]; remotes: string[]; tags: string[]; stashes: { name: string; subject: string }[]; worktrees: RepositoryWorktree[] };
function RepositoryPanelBody({ ctx }: RepositoryPanelProps) {
  ...
  useEffect(() => { if (!ctx.theaterId) return; let cancelled = false; ctx.api.fetch("repository", "refs", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ theaterId: ctx.theaterId, subPath }) }).then((r) => r.ok ? r.json() as Promise<Refs> : Promise.reject()).then((value) => { if (!cancelled) setRefs(value); }).catch(() => undefined); return () => { cancelled = true; }; }, [ctx.api, ctx.theaterId, subPath]);
  ...
  useLayoutEffect(() => {
    ctx.requestExtraWidth?.(source === "changes" && selectedFile ? EXTENDED_EXTRA_WIDTH : null);
    return () => ctx.requestExtraWidth?.(null);
  }, [ctx.requestExtraWidth, selectedFile, source]);
  ...
  return (
    <div className="repository-unified"><SourceNav source={source} refs={refs} onSource={setSource} onRef={(ref) => { setRefFilter(ref); setSource("history"); }} /><div className="repository-source-content">
      <div hidden={source !== "history"}><HistoryPanel ctx={ctx} active={source === "history"} refFilter={refFilter} onClearRef={() => setRefFilter(null)} onWip={() => setSource("changes")} /></div>
      {source !== "changes" && source !== "history" ? <RefList source={source} refs={refs} onRef={(ref) => { setRefFilter(ref); setSource("history"); }} /> : null}
...
function RefList({ source, refs, onRef }: { readonly source: Source; readonly refs: Refs; readonly onRef: (ref: string) => void }) { const rows = source === "branches" ? [...refs.branches.map((item) => ({ label: item.name, ref: `refs/heads/${item.name}`, current: item.current })), ...refs.remotes.map((name) => ({ label: name, ref: `refs/remotes/${name}`, current: false }))] : source === "tags" ? refs.tags.map((name) => ({ label: name, ref: `refs/tags/${name}`, current: false })) : source === "stashes" ? refs.stashes.map((item) => ({ label: `${item.name} ${item.subject}`, ref: "", current: false })) : refs.worktrees.map((item) => ({ label: item.branch ?? item.name, ref: "", current: item.current })); return <div className="repository-ref-list">{rows.map((row) => <button type="button" key={row.label} className={row.current ? "is-current" : ""} disabled={!row.ref} onClick={() => onRef(row.ref)}>{row.label}{row.current && " ✓"}</button>)}</div>; }
