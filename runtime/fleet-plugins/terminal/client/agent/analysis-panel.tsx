...
const SUGGESTIONS = ["Walk me through how this session unfolded", "What is the agent doing right now?", "Flag anything I should review", "Draft a handoff brief"];
export function AnalysisPanel({ context }: { readonly context: OperationRenderContext }) {
  ...
  const submit = async (text: string) => {
    ...
    dispatch({ type: "sending", started: !state.started, text: trimmed }); setDraft("");
  ...
  return <>
    ...
    <section className="session-analyst__artifacts" aria-label="Artifacts"><header><strong>Artifacts · sandboxed HTML</strong><button type="button" onClick={() => dispatch({ type: "clear-artifacts" })} disabled={!state.artifacts.length}>Clear</button></header>{state.artifacts.map((artifact) => { const srcdoc = safeArtifactSrcdoc(artifact.html); return srcdoc ? <article key={artifact.id}><header>{artifact.title}<span>Sandboxed</span></header><iframe title={artifact.title} srcDoc={srcdoc} sandbox="allow-scripts" /></article> : null; })}</section>
  </>;
}
