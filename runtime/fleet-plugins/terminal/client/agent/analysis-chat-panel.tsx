...
export function AnalystChatPanel({ context }: { readonly context: OperationRenderContext }) {
  ...
  return (
    <section className="session-analyst__chat-pane" aria-label="Session Analyst chat">
      ...
      <div className="session-analyst__selectors"><label className="session-analyst__select">CLI<select aria-label="Analysis CLI" disabled={state.started || !state.catalog} value={state.cliId} onChange={(event) => dispatch({ type: "select-cli", cliId: event.target.value })}>{state.catalog?.clis.map((item) => <option key={item.cliId} value={item.cliId} disabled={!item.available}>{item.label}</option>)}</select></label><label className="session-analyst__select">Model<select aria-label="Analysis model" disabled={state.started || !model} value={state.model} onChange={(event) => dispatch({ type: "select-model", model: event.target.value })}>{cli?.models.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label><label className="session-analyst__select">Effort<select aria-label="Analysis effort" disabled={state.started || !model || !model.effortLevels.length} value={state.effort} onChange={(event) => dispatch({ type: "select-effort", effort: event.target.value })}>{model?.effortLevels.length ? model.effortLevels.map((item) => <option key={item} value={item}>{item}</option>) : <option value="">n/a</option>}</select></label></div>
      ...
      <form className="session-analyst__composer" onSubmit={(event) => { event.preventDefault(); void submit(draft); }}><label className="session-analyst__sr-only" htmlFor={`analysis-${context.operationId}`}>Ask about this session</label><textarea id={`analysis-${context.operationId}`} placeholder="Ask about the session…" value={draft} onChange={(event) => setDraft(event.target.value)} disabled={state.busy} /><button type="submit" className="session-analyst__send" disabled={state.busy || !draft.trim()}>Send</button></form>
    </section>
  );
}
