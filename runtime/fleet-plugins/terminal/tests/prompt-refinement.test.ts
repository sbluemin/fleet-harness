...
import { PROMPT_REFINE_MAX_CHARS, type FleetPluginServerContext } from "@fleet-console/sdk/plugin";
import type { RouteHandler } from "@fleet-console/sdk/routing";
import { DEFAULT_EXPERIMENT_SETTINGS } from "@fleet-console/sdk/settings";
import { describe, expect, it, vi } from "vitest";

import { refineLaunchPrompt } from "../client/agent/experiments-api.js";
...
function harness() {
  ...
  const request = async (value: unknown) => {
    body = value;
    await route!({ req: { method: "POST" } as http.IncomingMessage, res: {} as http.ServerResponse, pathname: "/plugins/terminal/experiments/refine-prompt" } as Parameters<RouteHandler>[0]);
    return response!;
  };
  return {
