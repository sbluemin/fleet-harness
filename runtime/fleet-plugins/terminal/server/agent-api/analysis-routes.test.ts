...
describe("Session Analyst server contract", () => {
  ...
  it("wraps hostile artifact CSS with a validated host-owned canvas for every Console theme", async () => {
    ...
    for (const theme of ["instrument", "maritime", "carbon", "daywatch", "chartroom", "whites", "drydock"] as const) {
      ...
      const expectedColorScheme = ["daywatch", "chartroom", "whites", "drydock"].includes(theme) ? "light" : "dark";
      ...
      if (theme === "chartroom") expect(response.body).toContain("color-scheme:light!important");
