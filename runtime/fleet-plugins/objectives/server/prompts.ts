import { MAX_CONTEXT, type ObjectiveEditKind, type ObjectiveItem } from "./types.js";
 ...
 * 바뀐 내용 자체(미분류 단계 등)는 보드가 말한다. 바뀐 것 없이 말만 왔으면 말을 덧붙였다고만 한다.
 */
export function steerTurn(item: ObjectiveItem, language: PromptLanguage, context?: string): string {
  const what = editedWords(item, language);
  const note = quoted(context);
  const word = !what && note
    ? (language === "ko" ? `사람이 목표 \`${item.id}\` 에 말을 덧붙였습니다 — 보드를 다시 읽고 이어서 진행하세요.` : `The person added a note to objective \`${item.id}\` — read the board again and continue.`)
    : language === "ko"
 ...
 */
export function startTurn(item: ObjectiveItem, language: PromptLanguage, context?: string): string {
