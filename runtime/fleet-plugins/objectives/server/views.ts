...
import { assignModeOf, coordinatorMode, latestRecord, stepReady, type ObjectiveItem } from "./types.js";
...
/** 이 Operation 이 이 목표에서 맡은 자리 — 지휘관(목표 = 지휘관 Operation) 또는 담당(단계의 Operation). 둘 다 아니면 null. */
export function roleIn(item: ObjectiveItem, caller: ConsoleCaller | undefined): { role: "commander" } | { role: "assignee"; stepIndex: number; stepId: string } | null {
  if (caller?.kind !== "operation") return null;
  if (item.id === caller.operationId) return { role: "commander" };
  const index = item.steps.findIndex((step) => step.operationId === caller.operationId);
  return index >= 0 ? { role: "assignee", stepIndex: index, stepId: item.steps[index]!.id } : null;
}
...
export function createBoardViews(ctx: FleetPluginServerContext, store: ObjectiveStore) {
  ...
  const graph = (item: ObjectiveItem) => ({
    ...
    steps: item.steps.map((step, index) => ({
      ...
      // 담당은 이 목표를 읽을 수만 있다 — 지휘관이 보드에서도 그 사실을 본다.
      assignee: step.operationId ? { ...observe(step.operationId), session: step.sessionName, access: "read-only" as const } : null,
    })),
  });
