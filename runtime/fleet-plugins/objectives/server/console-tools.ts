...
export function createObjectiveConsoleTools(ctx: FleetPluginServerContext, store: ObjectiveStore, launch: LaunchService = createLaunchService(ctx, store)): readonly PluginMcpTool[] {
  ...
  const tool: PluginMcpTool = {
    ...
    description: "The Objectives board of a Theater, as the person sees it. Every agent Operation of the Theater is a virtual objective, while board-created objectives get a Commander Operation on first execution. Read with view groups | objectives (filter today|due|all|agent) | objective (note = the brief, attachments, missions, criteria = success criteria, members). Write with add: title; optional note (the brief), criteria (success-criterion sentences); at most 10 adds per caller per 10 minutes. The new objective carries no missions and follows the calling Operation's group; importance, due dates and grouping stay the person's acts on the screen. The new objective does not create an Operation until the person presses Plan or Commence. Carrying an objective out — planning, mustering members, completing missions, marking criteria — belongs to its Commander through the fleet-objectives tools, not here. Completing an objective and editing its brief after creation are the person's acts on the screen.",
