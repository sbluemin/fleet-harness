...
export function createObjectiveRoutes(ctx: FleetPluginServerContext, store: ObjectiveStore, launch: LaunchService = createLaunchService(ctx, store)): readonly ObjectiveRoute[] {
  ...
  return [
    ...
    { name: "objective/create", method: "POST", summary: "Create an objective together with its dormant Commander Operation.", handler: json(createObjectiveSchema, async ({ language, theaterId, title, groupId, note, important, dueDate, today, missions, viewMode }) => objective(await launch.create({ theaterId, title, groupId: groupId ?? null, note, important, dueDate, today, missions, viewMode }, { language }))) },
