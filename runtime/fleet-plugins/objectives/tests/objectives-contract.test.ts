...
import { adoptLegacyData, createObjectiveStore, ObjectiveStoreError } from "../server/store.js";
import type { ObjectiveItemEvent } from "../server/types.js";

/**
 * 목표의 필수 계약 — 실행(시작 한 번이 조율자와 담당 세션을 이름 붙여 한꺼번에 띄우고 이름을 조율자에게 알린다),
 * 저장 무결성(완료가 슬롯을 놓고 되돌리기가 복원, 순환 거절, 계획이 잠긴 단계를 보존), 권한 경계(조율자만 완료·계획,
 * 계보 밖 연결 거절, 같은 슬롯의 겹친 시작 거절), 사건 방송(쓰기 한 번에 objectives:item 한 프레임), 사람이 더한 단계는 지휘관이 자리를
 * 정하기 전까지 준비되지 않음, 메모 첨부는 이미지 머리 바이트로만 받고 항목과 함께 지워짐, 단계 기록은 다시 완료해도 쌓이고
 * 옛 결과는 첫 기록으로 옮겨지며 지휘관의 산문 요약은 거절됨.
 */
...
function harness() {
  ...
  const store = createObjectiveStore({ dir, emit: (event) => events.push(event) });
    ...
    host: {
      ...
      consoleControl: {
        request: async (input: { kind: string; operationId?: string; text?: string; title?: string; sessionName?: string; viewMode?: string; disableSubagents?: boolean }) => {
          if (input.kind === "send") { sent.push({ operationId: input.operationId!, text: input.text! }); return { id: "r", requestId: "r", caller: { kind: "plugin", pluginId: "objectives" }, input, status: "running", createdAt: "", updatedAt: "", expiresAt: "", operationId: input.operationId }; }
          ...
          return { id: "r", requestId: "r", caller: { kind: "plugin", pluginId: "objectives" }, input, status: "running", createdAt: "", updatedAt: "", expiresAt: "", operationId: id };
        },
        observe: () => null,
      },
...
describe("Objectives contract", () => {
  it("keeps an item in its Commander's Operation group: follows the Commander, reconciles on start, and moves the Commander when the item moves", async () => {
    ...
    const commander = (await launch.startCoordinator(item.id)).operationId;
    const worker = (await launch.delegateStep(item.id, item.steps[0]!.id)).operationId;
    await launch.complete(item.id, "human");
    // 사이드바에서 (완료된 항목의) 지휘관을 옮기면 항목이 따라간다; 담당만 옮긴 것은 항목을 움직이지 않는다.
    ctx.host.operations.patch(commander, { groupId: "g-done" });
    expect(store.find(item.id)!.groupId).toBe("g-done");
    ctx.host.operations.patch(worker, { groupId: "g-other" });
    expect(store.find(item.id)!.groupId).toBe("g-done");
    // 이 동기화 전에 갈라진 항목은 기동 때 지휘관의 그룹으로 맞춰진다.
    store.patch(item.id, { groupId: null });
    sync.reconcile();
    expect(store.find(item.id)!.groupId).toBe("g-done");
    // 목표에서 항목을 옮기면 지휘관과 담당이 따라가고, 되돌아온 사건은 항목을 다시 쓰지 않는다.
    const moved = store.patch(item.id, { groupId: "g-review" });
    const writes = store.find(item.id)!.updatedAt;
    sync.itemRegrouped(moved);
    expect([operations.get(commander)!.groupId, operations.get(worker)!.groupId]).toEqual(["g-review", "g-review"]);
  ...
  it("starts the coordinator and one named assignee session per step at once, and keeps storage consistent across completion, reopen and re-plan", async () => {
    ...
    // 새 단계의 기본은 「지휘관 직접」— 담당을 띄우려면 배정이 있어야 한다.
    ...
    // 시작 한 번 — 지휘관만 이름 붙은 CLI 세션으로 뜬다. 담당은 지휘관이 위임하는 순간에 프롬프트 없이 뜬다.
    const started = await launch.startCoordinator(item.id);
    const head = `objective-${item.id.slice(0, 6)}`;
    expect(launches.map((entry) => entry.sessionName)).toEqual([`${head}-cmdr`]);
    expect(launches[0]!.viewMode).toBe("terminal");
    expect(launches[0]!.text).toContain(item.id);
    expect(started.item.slot?.sessionName).toBe(`${head}-cmdr`);
    const delegated = await launch.delegateStep(item.id, a!.id);
    expect(delegated.session).toBe(`${head}-mission-1`);
    expect(launches.at(-1)).toMatchObject({ sessionName: `${head}-mission-1`, viewMode: "terminal", disableSubagents: true });
    ...
    // 단계는 편성 순으로 선다 — 계획이 뒤에 적은 선행(y)이 그 후속(x)보다 앞으로 온다.
    ...
    const reloaded = createObjectiveStore({ dir, emit: () => undefined });
    ...
    const legacy = createObjectiveStore({ dir, emit: () => undefined }).find("legacy-item")!.steps[0]!;
  ...
  it("lets only the Commander plan and mark steps, keeps completion and linking human-only, and refuses overlapping starts", async () => {
    ...
    // 계획은 담당을 띄우지 않는다 — 지휘관이 위임할 때 뜬다.
    expect(launches).toEqual([]);
    const delegated = await call({ step: { itemId: item.id, index: 0, delegate: true } }, { kind: "operation", operationId: "coord" });
    expect(delegated.structuredContent.session).toBe(`objective-${item.id.slice(0, 6)}-mission-1`);
    expect(launches.map((entry) => entry.sessionName)).toEqual([`objective-${item.id.slice(0, 6)}-mission-1`]);
    // 사람이 선행 없이 더한 단계는 미분류 — 지휘관이 자리를 정하기 전까지 준비되지 않는다. 지휘관의 step after 가 자리를 정한다.
    ...
    // 지휘관이 읽기 전에 사람이 바꾼 보드(edited)로는 계획도 검토 요청도 쓸 수 없다 — 옛 보드로 한 일이 사람의 편집을 덮거나 지나치지 않는다.
    ...
    // 목표 자체의 완료는 AI 도구에 없다 — 사람의 완료는 매핑을 남긴다.
  ...
  it("carries items and attachments saved under the old To-do plugin id over to the new data directory once", () => {
    ...
    dirs.push(base);
    const legacyRoot = path.join(base, "plugins", "todo");
    const root = path.join(base, "plugins", "objectives");
    const legacy = createObjectiveStore({ dir: path.join(legacyRoot, "theaters"), emit: () => undefined });
    const saved = legacy.create({ theaterId: "t1", title: "Carry me", steps: [{ text: "a" }] });
    const png = Buffer.from("89504e470d0a1a0a0000000d4948445200000002000000030806000000", "hex");
    const { attachment } = legacy.attachmentAdd(saved.id, { name: "shot.png", type: "image/png", data: png });

    adoptLegacyData(legacyRoot, root);
    const moved = createObjectiveStore({ dir: path.join(root, "theaters"), emit: () => undefined });
    const item = moved.get("t1", saved.id);
    expect(item?.title).toBe("Carry me");
    expect(fs.readFileSync(moved.attachmentPath(item!, attachment))).toEqual(png);
    expect(fs.existsSync(legacyRoot)).toBe(false);

    // 새 자리가 이미 있으면 옛 자리를 덮거나 합치지 않는다.
    fs.mkdirSync(legacyRoot, { recursive: true });
    fs.writeFileSync(path.join(legacyRoot, "marker"), "old");
    adoptLegacyData(legacyRoot, root);
    expect(fs.readFileSync(path.join(legacyRoot, "marker"), "utf8")).toBe("old");
    expect(moved.get("t1", saved.id)?.title).toBe("Carry me");
  });
});
