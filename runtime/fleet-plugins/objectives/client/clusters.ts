 ...
 * 목표 → 호스트 묶음 서술자.
 *
 * 조율자 슬롯이 뿌리, 담당 슬롯이 있는 단계가 구성원이다. 완료된 목표도 묶음으로 남는다 — 연결됐던 Operation 은
...
function slotsOf(item: ObjectiveItem): Slotted {
...
export function clustersOf(items: readonly ObjectiveItem[], activity: Map<string, string>): OperationCluster[] {
  ...
  for (const item of items) {
    ...
    // 지휘관 Operation 이 살아 있으면 묶음이 선다 — 단계 Operation 이 하나도 없어도, 단계가 하나뿐이어도 띠는 같은 모양이다.
