...
import { coordinatorMode, isLoose, stepDepths, stepReady, unseenRecords, wouldCycle, type ObjectiveItem } from "../server/types.js";
 ...
 * 사람이 선행 없이 더한 미분류 단계는 그래프 아래 「미분류」 칸에 따로 선다 — 지휘관이 자리를 정하거나, 사람이 여기서 이으면 올라간다.
 */
...
interface GraphProps {
  ...
  /** 이 단계의 선행을 사람이 바꿀 수 있는가 — 지휘관이 일하는 동안은 시작 전 단계만. 없으면 모두. */
...
export function CoordinationGraph({ item, t, modeLabel, onToggleEdge, onCycle, operationTitle, zoom = false, vertical = false, onZoom, canEdit, focusStepId = null, onFocusStep }: GraphProps) {
  ...
  // 첫 열의 x — 확대본은 뿌리(지휘관)와 첫 단계 사이를 넓혀 긴 제목이 뿌리 라벨을 덮지 않게 한다.
  ...
  if (vertical) return (
    <div className="objectives-dag-box objectives-dag-vertical">
      ...
      {steps.map((step, index) => <div
        key={step.id}
        className={`objectives-dag-row${focused?.id === step.id ? " is-focus" : focused?.after.includes(step.id) ? " is-pre" : ""}`}
    ...
    <div className={`objectives-dag-box${onZoom ? " can-zoom" : ""}${zoom ? " is-zoom" : ""}`}>
      ...
      >
        {steps.map((step) => {
          ...
          return (
            <g key={`edges-${step.id}`}>
              {step.after.length === 0 && !loose.includes(step) ? <path className="objectives-edge is-root" d={`M${root.x + 9},${root.y} C${root.x + 30},${root.y} ${p.x - 30},${p.y} ${p.x - 9},${p.y}`} /> : null}
              {step.after.map((parentId) => {
                ...
                return (
                  <path
                    ...
                    className={`objectives-edge${byId.get(parentId)?.done ? " is-met" : ""}${focused?.id === step.id ? " is-hot" : ""}`}
                    data-from={parentId}
                    data-to={step.id}
        ...
        {steps.map((step, index) => {
          ...
          return (
            <g
              key={step.id}
              className={`objectives-node ${cls}${drag?.over === step.id ? " is-over" : ""}${focused?.id === step.id ? " is-focus" : focused?.after.includes(step.id) ? " is-pre" : ""}`}
              data-k={step.id}
              tabIndex={0}
              role="button"
              aria-label={t("objectives.graph.nodeAria", { index: index + 1, text: step.text })}
            ...
            >
              ...
              {unseenRecords(step) > 0 ? <circle className="objectives-node-unseen" cx={p.x + 7} cy={p.y - 7} r={3} /> : null}
      ...
      {steps.length > 1 ? <div className="objectives-dag-hint">{t("objectives.graph.hint")}</div> : null}
      <span hidden>{mode}</span>
    </div>
  );
}
