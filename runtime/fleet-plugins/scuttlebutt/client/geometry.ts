...
/**
 * 도구모음에 둔 부관의 시트 자리. 글리프의 오른쪽 변에 맞추고, 글리프 아래로 내려온다 — 새의 위치가
 * 아니라 도구모음이 닻이라 항상 같은 자리다. 도구모음이 화면 아래(Zen 작업 표시줄 트레이)에 서면
 * 아래에 자리가 없으므로 위로 연다. 높이는 화면의 80%까지만 쓴다: 그 이상은 답이 아니라 벽이다.
 */
export function placeDockedCard(
  ...
  const left = clamp(glyph.left + glyph.width - card.width, EDGE, Math.max(EDGE, viewport.width - card.width - EDGE));
  const cap = viewport.height * 0.8;
  const top = glyph.top + glyph.height + GAP;
  const roomBelow = viewport.height - top - EDGE;
  const roomAbove = glyph.top - GAP - EDGE;
  if (roomBelow < Math.min(card.height, cap) && roomAbove > roomBelow) {
    return {
      side: "above",
      left,
      bottom: viewport.height - glyph.top + GAP,
