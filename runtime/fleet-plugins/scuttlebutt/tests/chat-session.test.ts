...
describe("Scuttlebutt Console Agent integration", () => {
  it("requests only web tools, preserves the conversation, and maps completion and cancellation", async () => {
    ...
    const granted = () => true;
    const operational = new ChatSession({ agent, admiral: "bori", consoleUse: { custom: [], consoleUse: { tools: ["console_launch"], allowControl: true }, promptAddendum: "Console execution enabled" }, computerUse: { computerUse: { enabled: granted }, promptAddendum: "Computer Use enabled" } });
    await operational.start();
    expect(options.tools).toMatchObject({ builtins: ["WebSearch", "WebFetch"], consoleUse: { tools: ["console_launch"], allowControl: true }, aiGateway: true, computerUse: { enabled: granted } });
