...
When the change is not a feature-level product delta — refactors, boundary gates, doctrine and prompt edits, test repairs, release tooling, and similar — write no fragment and no `no-changelog` declaration.
...
2. Write the new or amended fragment chosen in Phase 0 (recording an amended filename for the Phase 2 declaration) and validate with `node scripts/compile-changelog-fragments.mjs --check`.
