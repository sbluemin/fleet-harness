...
Use `& $ab ...` for every subsequent command. If Windows x64 emulation cannot execute the official binary, stop and report the environment block. Do not rename an x64 binary to appear ARM64, use a third-party build, or claim agent-browser ran when another driver was used.
