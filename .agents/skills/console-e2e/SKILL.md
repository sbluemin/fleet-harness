...
- **A fresh isolated instance is never modal-free — clear its auto-modals before testing a no-modal path**: with no Theater it renders the commissioning onboarding overlay (`role="dialog" aria-modal="true"`), and on first load the What's New modal auto-opens (also `aria-modal`). Both satisfy `[aria-modal="true"]:not([hidden])`, so any check of a global shortcut's modal-boundary guard — or of the shortcut firing at all — is confounded unless you clear them first: seed at least one Theater via `state.json` (dismisses commissioning) **and** dismiss What's New (`document.querySelector('.whatsnew-close')?.click()`), then assert `document.querySelectorAll('[aria-modal="true"]:not([hidden])').length === 0` before driving the shortcut. Otherwise a shortcut correctly suppressed behind the auto-modal reads as a false negative. (Confirmed while verifying the sidebar/rail toggle shortcuts, 2026-07-12.)
