...
Every release-impacting PR must include exactly one fragment named after its own branch, unless it intentionally carries the `no-changelog` label. The branch already exists when the work starts, so the fragment is staged and committed **together with the change it describes** — there is no second commit and nothing to wait for. Read the filename from `node scripts/compile-changelog-fragments.mjs --name-for-branch`; never derive it by hand.
