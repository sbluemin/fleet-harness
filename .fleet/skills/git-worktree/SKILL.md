...
- **Carrier edits inside the new worktree** — after `create`, if you delegate file edits to a carrier (`carrier_dispatch`), remember the carrier's cwd is the **main checkout**, not this worktree. Pass the **absolute worktree path** explicitly in the dispatch, instruct it to prefix every repo-relative path with that root and to write only there, forbid it from running `build`/`typecheck` (the Admiral verifies inside the worktree), and after each return run `git -C <main-checkout> status --short` to confirm the main checkout stayed clean. Treat the carrier's reported `workspaceChanges` (window-approx) as unreliable — trust the real `git status`.
