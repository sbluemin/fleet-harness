...
version: 3
rawSourceRef: "raw/2026-05-16-prd-carrier-persona-extraction-source-90383528.md"
---
## Overview
...
- **As a** 시스템, **when** 캐리어가 executor 세션을 시작할 때, **then** 글로벌로 등록된 위키 읽기 도구는 자동 상속되고, 상태 조회 및 쓰기/린트 도구는 페르소나에 명시적으로 선언된 경우에만 노출됩니다.
...
- **글로벌 기본 도구 (읽기 전용)**: 위키 읽기 도구 4종(`wiki_briefing`, `wiki_orient`, `wiki_read`, `wiki_resolve`)은 글로벌로 등록되어 모든 캐리어에 자동 상속됩니다. `wiki_query`는 `stage_answer_page` 모드에서 패치 큐를 변경할 수 있어 글로벌이 아닌 명시적 권한 선언 채널에 속합니다.
- **명시적 권한 선언 (상태/쓰기)**: `carrier_jobs`, `wiki_query`, 위키 쓰기/린트 도구(`wiki_ingest`, `wiki_patch_edit`, `wiki_drydock`) 등 자동 상속에서 제외된 도구는 페르소나 메타데이터에 명시적으로 열거해야 합니다.
...
- [ ] 지식 관리 캐리어(Chronicle)가 페르소나에 선언한 전용 위키 도구(쓰기/린트)에 접근 가능한가?
- [ ] 모든 캐리어가 글로벌 위키 읽기 도구 4종에 접근 가능한가?
- [ ] `carrier_jobs`는 페르소나에 명시적으로 선언한 캐리어에게만 노출되는가?
